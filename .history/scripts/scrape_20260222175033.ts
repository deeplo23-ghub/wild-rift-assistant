  ...
  if (!sampleMode && uniqueChampions.length < SCRAPER_CONFIG.targetChampionCount) {
    ...
    try {
      ...
      for (const link of homepageLinks) {
        ...
        const name = link.text.replace(/Build Guide|Wild Rift|Build/gi, "").trim();
        if (!name || name.length < 2) continue;

        const normalizedSearchName = name.toUpperCase().replace(/[^A-Z0-9]/g, "");
        if (!uniqueChampions.some(c => c.name.toUpperCase().replace(/[^A-Z0-9]/g, "") === normalizedSearchName)) {
  ...
  try {
    ...
    const champions = pageResults.map(pr => {
      ...
      return {
        ...
        winrate: pr.winrate || tierData?.winrate || 50.0,
        pickRate: pr.pickRate || 0.1,
        banRate: pr.banRate || tierData?.banRate || 0.1,
