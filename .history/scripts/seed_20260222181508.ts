...
async function main() {
  ...
  for (const raw of rawResult.data.champions) {
    try {
  ...
  logger.info("── Step 5: Seeding database ──");

  const connectionString = process.env.DATABASE_URL;
  if (!connectionString) {
    logger.error("DATABASE_URL not set. Use --dry-run or set DATABASE_URL in .env");
    process.exit(1);
  }

  // Dynamic import to avoid requiring DB connection at validation time
  ...
  const PrismaPg = (adapterModule as any).PrismaPg || (adapterModule as any).default?.PrismaPg;
  ...
  try {
    ...
    for (const champ of normalizedChampions) {
