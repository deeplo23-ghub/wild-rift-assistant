...
async function main() {
  ...
  if (jobId) await updateSyncStatus(jobId, { progress: 5.12, message: "Scanning website infrastructure..." });
  ...
  if (jobId) await updateSyncStatus(jobId, { progress: 12.45, message: "Fetching global tier lists..." });
  ...
  if (jobId) await updateSyncStatus(jobId, { progress: 28.92, message: "Consolidating champion roster..." });
  ...
  if (jobId) await updateSyncStatus(jobId, { progress: 38.67, message: "Optimizing scrape queue..." });
  ...
  logger.info(`Queueing ${pagesToScrape.length} pages (skipping ${uniqueChampions.length - pagesToScrape.length} cached)`);
  ...
  try {
    pageData = await scrapeAllChampionPages(pagesToScrape, logger, browser, false, jobId);
  } finally {
    await browser.close();
  }
  ...
  if (jobId) await updateSyncStatus(jobId, { progress: 92.45, message: "Merging cross-referenced data..." });
  // ── Step 7: Write output ───────────────────────────────────────────────
  if (jobId) await updateSyncStatus(jobId, { progress: 94.88, message: "Finalizing data structures..." });
