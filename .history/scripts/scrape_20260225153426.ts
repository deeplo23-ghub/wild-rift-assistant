...
async function main() {
  ...
  const pagesToScrape = uniqueChampions.slice(0, limit).filter(champ => {
    ...
    // Only skip if we have threats data (meaning the page was actually scraped before)
    const hasData = cached && (cached.extremeThreats?.length > 0 || cached.extremeSynergies?.length > 0);
    return !hasData;
  });
