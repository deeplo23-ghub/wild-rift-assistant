...
async function main() {
  ...
  const allChampions = rawChampions.map(c => ({
    id: c.id,
    name: c.name,
    roles: c.roleTags.map(r => r.role as Role),
    winrate: Number(c.winrate),
    pickRate: Number(c.pickRate),
    banRate: Number(c.banRate),
    tier: c.tier as any,
    damageProfile: {
      ad: Number(c.damageProfile?.ad ?? 0),
      ap: Number(c.damageProfile?.ap ?? 0),
      true: Number(c.damageProfile?.trueDamage ?? 0),
    },
    ...
    tags: c.tags.map(t => t.tag.id as any),
    iconUrl: c.iconUrl || "",
  }));
