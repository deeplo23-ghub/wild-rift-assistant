...
export async function scrapeTierList(logger: Logger): Promise<TierListEntry[]> {
  ...
  try {
    ...
    // Click through all role tabs to ensure all champion cards are rendered in DOM
    logger.info("Clicking role tabs to load all champions...");
    const roleLabels = ['mid', 'solo', 'jungle', 'duo', 'support'];
