...
export async function scrapeAllChampionPages(
  champions: { id: string, name: string, pageUrl: string }[],
  logger: Logger,
  browser: Browser,
  ...
  if (dryRun) {
    logger.info("DRY RUN: Would scrape champion pages", { count: champions.length });
    return [];
  }

  logger.info("Starting champion page scrape", { count: champions.length });
  ...
  for (let i = 0; i < champions.length; i++) {
    ...
    if (jobId) {
      const progress = 40 + (i / champions.length) * 50;
      await updateSyncStatus(jobId, {
        progress: parseFloat(progress.toFixed(2)),
        message: `Scraping ${champ.name} [${i + 1}/${champions.length}]...`
      });
    }

    logger.info(`Scraping [${i + 1}/${champions.length}] ${champ.name}`);

    const data = await withRetry(
      () => scrapeChampionPage(browser, champ.pageUrl, logger),
      champ.pageUrl
    );
