          ...
          <AnimatePresence>
            {activeJobId && syncJob && (
              <motion.div
                ...
                className="fixed bottom-10 left-1/2 z-[100] w-[450px] bg-black/40 backdrop-blur-3xl border border-white/10 rounded-2xl p-6 shadow-[0_30px_60px_rgba(0,0,0,0.6)] ring-1 ring-white/10 overflow-hidden"
              >
                ...
                <div className="flex items-center gap-6 relative z-10">
                  <div className={cn(
                    ...
                    syncJob.status === "COMPLETED" ? "bg-emerald-500/20 text-emerald-400" :
                    syncJob.status === "FAILED" ? "bg-red-500/20 text-red-400" :
                  ...
                  <div className="flex-1 min-w-0 flex items-baseline justify-between gap-4">
                    <div className="min-w-0 flex-1">
                      <h4 className="text-[10px] font-black uppercase tracking-[0.3em] text-white/50 mb-1">
                        {syncJob.status === "COMPLETED" ? "SYSTEM UPDATED" :
                         syncJob.status === "FAILED" ? "EXTERNAL ERROR" :
                    ...
                    <div className="flex items-start shrink-0">
                      <AnimatePresence mode="popLayout">
                        <motion.span
                          ...
                          className="text-4xl font-black font-mono tracking-tighter text-white tabular-nums leading-none"
                        >
                          {syncJob.progress}
                        </motion.span>
                      </AnimatePresence>
                      <span className="text-sm font-black text-white/30 ml-1 mt-1 font-mono">%</span>
                    </div>
                  </div>
                  ...
                  {(syncJob.status === "COMPLETED" || syncJob.status === "FAILED") && (
                    <button 
                      ...
                      className="ml-2 w-8 h-8 rounded-full flex items-center justify-center text-zinc-500 hover:text-white hover:bg-white/5 transition-all"
                    >
                ...
                <div className="mt-6 w-full h-[3px] bg-white/5 rounded-full overflow-hidden relative">
                  <motion.div 
                    initial={{ width: 0 }}
                    animate={{ width: `${syncJob.progress}%` }}
                    transition={{ duration: 0.8, ease: "circOut" }}
                    className={cn(
                      "h-full relative z-10 shadow-[0_0_15px_rgba(59,130,246,0.3)]",
                  ...
                  {/* Lead glow effect */}
                  <motion.div
                    animate={{ left: `${syncJob.progress}%` }}
                    transition={{ duration: 0.8, ease: "circOut" }}
                    className="absolute top-0 h-full w-20 bg-gradient-to-r from-transparent via-white/20 to-transparent -translate-x-full blur-md"
                  />
                </div>
              </motion.div>
            )}
