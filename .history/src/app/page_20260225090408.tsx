...
export default function DraftPage() {
  ...
  useEffect(() => {
    const logoFinishTime = 4100; // 0.5s delay + 3.6s logo anim
    const waitTime = 1000; // 1s wait after logo
    const totalDuration = logoFinishTime + waitTime;
    ...
    const interval = setInterval(() => {
      ...
      if (elapsed < logoFinishTime) {
        // Phase 1: Logo is drawing (0% -> 85%)
        calculatedProgress = (elapsed / logoFinishTime) * 85;
      } else {
        // Phase 2: Logo finished, final 1s stretch (85% -> 100%)
        const waitElapsed = elapsed - logoFinishTime;
        calculatedProgress = 85 + (Math.min(1, waitElapsed / waitTime) * 15);
      }

      // Add "Stumble" Jitter for realism
      const jitter = (Math.sin(elapsed / 100) * 0.5);
      setProgress(prev => {
        const next = Math.max(prev, calculatedProgress + jitter);
  ...
  return (
    <AnimatePresence mode="wait">
          ...
          <div className="flex flex-col items-center gap-12 relative">
            ...
            >
              ...
              <div className="flex justify-between w-full px-0.5">
                <span className="text-[9px] font-black tracking-[0.2em] text-blue-500/60 uppercase">LOADING...</span>
                <span className="text-[10px] font-mono tabular-nums text-blue-400/80 font-bold">
                  {Math.round(progress)}%
                </span>
              </div>
            </motion.div>
      ...
      <footer className={cn(
        ...
        settings.disableTransparency ? "bg-zinc-900" : "bg-black/40"
      )}>
