          ...
          <AnimatePresence>
            {activeJobId && syncJob && (
              <motion.div
                ...
                className="fixed bottom-10 left-1/2 z-[100] w-96 bg-black/40 backdrop-blur-2xl border border-white/10 rounded-2xl p-5 shadow-[0_20px_50px_rgba(0,0,0,0.5)] ring-1 ring-white/5"
              >
                <div className="flex items-center gap-4 mb-3">
                  <div className={cn(
                    ...
                    {syncJob.status === "COMPLETED" ? <CheckCircle2 className="w-5 h-5" /> :
                     syncJob.status === "FAILED" ? <AlertCircle className="w-5 h-5" /> :
                ...
                <div className="flex justify-between items-center text-[8px] font-black text-zinc-600 uppercase tracking-widest">
                  <span>Progress</span>
                  <span className="font-mono text-zinc-400">{syncJob.progress}%</span>
                </div>
              </motion.div>
            )}
          </AnimatePresence>
