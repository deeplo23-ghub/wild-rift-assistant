          ...
          <AnimatePresence>
            {activeJobId && syncJob && (
              <motion.div
                ...
                className="fixed bottom-10 left-1/2 z-[100] w-[480px] bg-zinc-950/40 backdrop-blur-3xl border border-white/10 rounded-2xl p-6 shadow-[0_40px_100px_rgba(0,0,0,0.7)] ring-1 ring-white/10 overflow-hidden"
              >
                {/* Accent Glow */}
                <div className="absolute -top-12 -right-12 w-32 h-32 bg-blue-500/10 blur-[60px] rounded-full pointer-events-none" />
                
                <div className="flex items-center gap-6 relative z-10">
                  {/* Icon Section */}
                  <div className={cn(
                    "w-14 h-14 rounded-2xl flex items-center justify-center shrink-0 border border-white/5 transition-colors duration-500",
                    syncJob.status === "COMPLETED" ? "bg-emerald-500/10 text-emerald-400 border-emerald-500/20" :
                    syncJob.status === "FAILED" ? "bg-red-500/10 text-red-400 border-red-500/20" :
                    "bg-white/5 text-blue-400"
                  )}>
                    {syncJob.status === "COMPLETED" ? <CheckCircle2 className="w-7 h-7" /> :
                     syncJob.status === "FAILED" ? <AlertCircle className="w-7 h-7" /> :
                     <Loader2 className="w-7 h-7 animate-spin" />}
                  </div>

                  {/* Text & Percentage Section */}
                  <div className="flex-1 min-w-0">
                    <h4 className="text-[9px] font-black uppercase tracking-[0.4em] text-white/30 mb-2">
                      {syncJob.status === "COMPLETED" ? "SYSTEM CORE UPDATED" :
                       syncJob.status === "FAILED" ? "EXTERNAL SYNC FAILURE" :
                       "DATA SYNCHRONIZATION"}
                    </h4>
                    
                    <div className="flex items-center justify-between gap-6">
                      <div className="text-base text-zinc-100 font-bold truncate flex-1">
                        {syncJob.message || "Initializing engine..."}
                      </div>
                        ...
                        <div className="relative h-12 overflow-hidden flex items-center">
                          <AnimatePresence mode="popLayout">
                            <motion.span
                              ...
                              className="text-5xl font-black font-mono tracking-tighter text-white tabular-nums"
                            >
                              {syncJob.progress}
                            </motion.span>
                          </AnimatePresence>
                        </div>
                        <span className="text-lg font-black text-white/20 ml-1 font-mono">%</span>
                      </div>
                    </div>
                  </div>
                  ...
                  {(syncJob.status === "COMPLETED" || syncJob.status === "FAILED") && (
                    <button 
                      ...
                      className="w-10 h-10 rounded-xl flex items-center justify-center text-zinc-500 hover:text-white hover:bg-white/10 border border-transparent hover:border-white/10 transition-all shadow-sm"
                    >
                ...
                {/* Progress Bar */}
                <div className="mt-8 w-full h-[4px] bg-white/5 rounded-full overflow-hidden relative">
                  <motion.div 
                    initial={{ width: 0 }}
                    ...
                    className={cn(
                      ...
                      syncJob.status === "COMPLETED" ? "bg-emerald-500 shadow-[0_0_15px_rgba(16,185,129,0.4)]" :
                      syncJob.status === "FAILED" ? "bg-red-500 shadow-[0_0_15px_rgba(239,68,68,0.4)]" :
                  ...
                  <motion.div
                    ...
                    className="absolute top-0 h-full w-40 bg-gradient-to-r from-transparent via-white/10 to-transparent skew-x-12 z-20"
                  />
                </div>
              </motion.div>
            )}
