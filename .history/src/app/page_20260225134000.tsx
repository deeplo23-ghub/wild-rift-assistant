...
import { Loader2, RotateCcw, Info, Settings2, BarChart3, Ban, Database } from "lucide-react";
...
export default function DraftPage() {
  ...
  const allyPicksCount = useDraftStore((state) => Object.values(state.ally).filter(v => v.championId).length);
  const enemyPicksCount = useDraftStore((state) => Object.values(state.enemy).filter(v => v.championId).length);
    ...
    <AnimatePresence mode="wait">
      {(loadingChamps || loadingMatrix || isAnimating) ? (
        <motion.div 
          ...
          transition={{ duration: settings.disableIntro ? 0.2 : 1, ease: [0.22, 1, 0.36, 1] }}
