...
interface DraftStore extends DraftState {
  ...
  focusedRole: Role | null; // null if in Ban Mode or if no slot focused
...
export const useDraftStore = create<DraftStore>((set, get) => ({
  ...
  addBan: (side, championId) => {
    set((state) => {
      ...
      // Alternate teams or stay until 5? Usually users do it team by team or alternating.
      // Let's alternate team if the current one has more than or equal to the other.
      if (nextBans[side].length > nextBans[side === TeamSide.Ally ? TeamSide.Enemy : TeamSide.Ally].length) {
        nextSide = side === TeamSide.Ally ? TeamSide.Enemy : TeamSide.Ally;
      }
