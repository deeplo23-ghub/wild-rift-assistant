...
interface DraftStore extends DraftState {
  ...
  // Interactive focus (Role = null means banning mode for that team)
...
export const useDraftStore = create<DraftStore>((set, get) => ({
  ...
  addBan: (side, championId) => {
    set((state) => {
      ...
      if (bans.length >= 5) return state; // Limit to 5 per side
      // WR Blind Ban allows duplicates across teams, but usually not in same team list
      if (bans.includes(championId)) return state; 
  ...
  autoPickChampion: (championId) => {
    ...
    // No automatic advance as per user request: "no automatic active slot switching"
    if (focusedRole) {
