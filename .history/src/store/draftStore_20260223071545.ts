...
export const useDraftStore = create<DraftStore>((set, get) => ({
  ...
  autoPickChampion: (championId) => {
    const { focusedSide, focusedRole } = get();
    if (focusedRole) {
      get().pickChampion(focusedSide, focusedRole, championId);
      
      // Auto-advance focus to next empty role
      const roles = [Role.Baron, Role.Jungle, Role.Mid, Role.Dragon, Role.Support];
      const currentIndex = roles.indexOf(focusedRole);
      const nextId = (currentIndex + 1) % roles.length;
      const nextRole = roles[nextId];
      
      // Rotate side if we finished a team (optional, for alternating picks)
      // For simplicity, let's just stay on the same team until manual switch or last role
      set({ focusedRole: nextRole });
    } else {
  ...
  resetDraft: () => {
    ...
    set({ scoredChampions: [], focusedSide: TeamSide.Ally, focusedRole: Role.Baron });
  },
}));
