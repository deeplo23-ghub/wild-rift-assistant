...
export const SCORE_COLORS = (score: number) => {
  ...
  if (score >= 87.5) return "text-yellow-400/90";
  if (score >= 85) return "text-yellow-500";
  if (score >= 82.5) return "text-amber-300";
  if (score >= 80) return "text-amber-400";

  // 70 - 77.5: Green range
  if (score >= 77.5) return "text-lime-400";
  if (score >= 75) return "text-lime-500";
  if (score >= 72.5) return "text-green-400";
  if (score >= 70) return "text-green-500";

  // 60 - 67.5: Transition range (Teal/Cyan)
  if (score >= 67.5) return "text-teal-400";
  if (score >= 65) return "text-teal-500";
  if (score >= 62.5) return "text-cyan-400";
  if (score >= 60) return "text-cyan-500";

  // 50 - 57.5: Red/Orange range
  if (score >= 57.5) return "text-orange-400";
  if (score >= 55) return "text-orange-500";
  if (score >= 52.5) return "text-red-400";
                  ...
                  <div className="flex gap-[10px] items-start">
                    <div className="flex flex-col items-center gap-1.5 shrink-0">
                      <div className="relative w-[76px] h-[76px] rounded-lg overflow-hidden border border-white/10 shadow-inner group-hover:border-blue-500/50 transition-colors">
                        <ChampionIcon name={champion.name} url={champion.iconUrl} className="w-full h-full transform transition-transform duration-300 group-hover:scale-110" />
                      </div>
      ...
      <ScrollArea className="flex-1 h-full min-h-0 bg-black/20 rounded-xl border border-white/5 p-5 shadow-inner">
        <div className="grid grid-cols-10 gap-2 justify-center items-start">
          {others.map((champion) => {
            ...
            const isBannedByOther = focusedSide === TeamSide.Ally ? enemyBans.has(champion.id) : allyBans.has(champion.id);
            ...
            return (
              <button
                ...
                className={cn(
                  ...
                  isUnavailable ? (isPicked ? "opacity-20 grayscale cursor-not-allowed" : "cursor-not-allowed") : ""
                )}
              >
                <div className="relative w-full aspect-square shadow-md group-hover:shadow-lg rounded-lg">
                  <div className={cn(
                    ...
                    <ChampionIcon 
                      ...
                      className={cn("w-full h-full transform transition-transform duration-300 group-hover:scale-110", (isUnavailable || isAnyBanned) && "grayscale", isPicked && "opacity-30", isAnyBanned && "opacity-50")} 
                    />
                    {isAnyBanned && (
