      ...
      <CardContent className="flex flex-col gap-3">
        {(Object.keys(ROLE_LABELS) as Role[]).map((role) => {
          ...
          return (
            <div 
              key={role}
              className="group relative flex items-center gap-4 p-3 rounded-xl border border-white/5 bg-zinc-800/40 transition-all duration-300 hover:border-blue-500/30"
            >
              <div className="w-12 h-12 rounded-lg bg-zinc-700/50 border border-white/10 flex items-center justify-center overflow-hidden shrink-0">
                {hasChampion ? (
                  ...
                  <div className="w-5 h-5 rounded-full bg-blue-500/20 border border-blue-500/30 animate-pulse" />
                )}
              </div>
