...
export const TeamPanel: React.FC<TeamPanelProps> = ({ side }) => {
  ...
  return (
    <div className="flex flex-col gap-4 h-full">
                ...
                return (
                    <button key={i} 
                      onClick={(e) => {
                          e.stopPropagation();
                          if (!champ) {
                             useDraftStore.getState().toggleBanMode(true);
                             setFocusedSlot(side, null);
                          } else {
          ...
          return (
            <div key={role} className="flex flex-col gap-1">
                ...
                >
                  ...
                  <div className="w-[76px] h-[76px] rounded-md bg-black/40 backdrop-blur-md border border-white/5 flex items-center justify-center overflow-hidden shrink-0">
                    {champion ? (
                      <ChampionIcon 
                        name={champion.name} 
                    ...
                    {champion ? (
                      <>
                        ...
                        <div className={cn("flex items-center gap-2", !isAlly && "flex-row-reverse")}>
                           ...
                           <span className="text-sm font-bold text-zinc-100 truncate leading-none">{champion.name}</span>
                           {bd && (
                             <>
                               <span className="text-cyan-400 font-bold text-xs leading-none">{Math.round(bd.finalScore)}</span>
                               <span className={cn("text-emerald-400 font-bold text-xs leading-none", isAlly ? "ml-auto" : "mr-auto")}>{champion.winrate.toFixed(1)}%</span>
                             </>
                           )}
                        </div>
                        
                        {/* Line 2: Headers */}
                        <div className={cn("flex items-center gap-3 text-[9px] font-black uppercase tracking-wider text-zinc-500", !isAlly && "flex-row-reverse")}>
                           <span className={cn("w-10", isAlly ? "text-left" : "text-right")}>Syn</span>
                           <span className="text-zinc-700/50">•</span>
                           <span className={cn("w-10", isAlly ? "text-left" : "text-right")}>Ctr</span>
                           <span className="text-zinc-700/50">•</span>
                           <span className={cn("w-10", isAlly ? "text-left" : "text-right")}>Weak</span>
                        </div>

                        {/* Line 3: Icons */}
                        <div className={cn("flex items-center gap-3", !isAlly && "flex-row-reverse")}>
                           <div className={cn("flex gap-0.5 w-10", !isAlly && "flex-row-reverse justify-end")}>
                             {getTopSynergies(champion).slice(0, 3).map(s => {
                               const c = allChampions.find(champ => champ.id === s.id);
                               return c ? <ChampionIcon key={c.id} name={c.name} url={c.iconUrl} className="w-[14px] h-[14px] rounded border border-white/10" /> : null;
                             })}
                           </div>
                           <span className="text-transparent text-[9px]">•</span>
                           <div className={cn("flex gap-0.5 w-10", !isAlly && "flex-row-reverse justify-end")}>
                             {getTopCounters(champion.id).slice(0, 3).map(c => {
                               const champ = allChampions.find(ch => ch.id === c.id);
                               return champ ? <ChampionIcon key={champ.id} name={champ.name} url={champ.iconUrl} className="w-[14px] h-[14px] rounded border border-white/10" /> : null;
                             })}
                           </div>
                           <span className="text-transparent text-[9px]">•</span>
                           <div className={cn("flex gap-0.5 w-10", !isAlly && "flex-row-reverse justify-end")}>
      ...
      <Card className="mt-2 border-white/5 bg-black/40 backdrop-blur-md overflow-hidden flex-1 shadow-2xl flex flex-col">
        ...
        <CardContent className="p-4 space-y-4 font-sans">
            ...
            {/* Insight Box */}
            <div className={cn("p-3 rounded-lg flex items-center gap-3 border", isAlly ? "bg-red-500/5 border-red-500/10 backdrop-blur-md" : "bg-black/40 backdrop-blur-md border-white/5")}>
