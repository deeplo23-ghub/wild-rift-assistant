                ...
                <div className="h-full overflow-y-auto pr-1 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none] flex flex-col gap-2 pb-8">
                    ...
                    <div className="flex flex-col gap-1.5 bg-black/20 p-2.5 rounded-lg border border-white/5">
                        ...
                        {ENGINE_METRICS.map(m => {
                            ...
                            return (
                                 <Tooltip open={settings.showTooltips ? undefined : false} delayDuration={0} key={m.key}>
                                     <TooltipTrigger asChild>
                                         <div 
                                             className={cn("flex items-center gap-3 group h-8 px-1.5 hover:bg-white/[0.03] rounded transition-colors duration-200", settings.showTooltips ? "cursor-help" : "cursor-default")}
