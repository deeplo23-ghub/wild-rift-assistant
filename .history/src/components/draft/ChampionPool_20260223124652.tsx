      ...
      {top3.length > 0 && !activeSearch && !isBanMode && (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
          {top3.map((champion, idx) => {
            ...
            const rankStyles = [
              { border: "border-yellow-400/50 shadow-[0_0_20px_rgba(250,204,21,0.2)] scale-[1.02] z-10 animate-glint animate-shimmer", tierGlow: "tier-glow-gold" },
              { border: "border-zinc-400/50 shadow-[0_0_15px_rgba(161,161,170,0.15)]", tierGlow: "tier-glow-silver" },
              { border: "border-orange-700/50 shadow-[0_0_15px_rgba(194,65,12,0.1)]", tierGlow: "tier-glow-bronze" }
            ][idx] || { border: "border-white/5", tierGlow: "" };
            ...
            return (
              <Card 
                ...
                className={cn(
                  ...
                  isUnavailable ? "opacity-40 grayscale pointer-events-none" : "hover:shadow-blue-500/10",
                  rankStyles.border
                )}
              >
                <CardContent className="p-[15px] flex flex-col gap-2">
                  ...
                  <div className="flex gap-[10px] items-start">
                    <div className="relative w-[76px] h-[76px] rounded-lg overflow-hidden border border-white/10 shrink-0 shadow-inner group-hover:border-white/30 transition-colors">
                      <ChampionIcon name={champion.name} url={champion.iconUrl} className="w-full h-full" />
                      
                      {/* Tier Badge Below (Glass) */}
                      <div className={cn(
                        "absolute bottom-0 inset-x-0 h-5 flex items-center justify-center backdrop-blur-xl border-t border-white/10 shadow-lg",
                        "bg-black/60",
                        champion.tier === "S+" ? "animate-shimmer" : "",
                        rankStyles.tierGlow
                      )}>
                        <span className={cn(
                          "text-[10px] font-black tracking-tighter block leading-none",
                      ...
                      {score && (
                        <div className="flex flex-col gap-[3px] text-[9px] mt-0.5">
                            ...
                            return (
                              <div key={item.label} className="flex justify-between items-center group/calc">
                                <div className="flex items-center gap-1">
                                   <span className="text-[7px] text-zinc-500 uppercase font-black tracking-widest leading-none">{item.label}</span>
                          ...
                          <div className="flex justify-between items-center">
                            <span className="text-[7px] text-zinc-400 uppercase font-black tracking-widest leading-none">Sub-Total</span>
