...
export function ChampionPool({ champions }: ChampionPoolProps) {
  ...
  const sortedChampions = useMemo(() => {
    return champions
      ...
      .sort((a, b) => {
        const isAUntouchable = pickedIds.has(a.id) || (isBanMode ? (focusedSide === TeamSide.Ally ? allyBans.has(a.id) : enemyBans.has(a.id)) : combinedBans.has(a.id));
        const isBUntouchable = pickedIds.has(b.id) || (isBanMode ? (focusedSide === TeamSide.Ally ? allyBans.has(b.id) : enemyBans.has(b.id)) : combinedBans.has(b.id));

        if (isAUntouchable && !isBUntouchable) return 1;
        if (!isAUntouchable && isBUntouchable) return -1;
            ...
            return (
              <Card 
                ...
                className={cn(
                  ...
                  idx === 0 ? "border-gold-shimmer scale-[1.05] z-10 shadow-[0_0_20px_rgba(234,179,8,0.2)]" : "border-white/5"
                )}
              >
                ...
                <div className="relative w-full aspect-square shadow-md group-hover:shadow-lg rounded-lg">
                  <div className={cn(
                    ...
                    <ChampionIcon 
                      ...
                      className={cn("w-full h-full", isUnavailable && "grayscale", isPicked && "opacity-30", isAnyBanned && "opacity-50")} 
                    />
                    {isAnyBanned && (
