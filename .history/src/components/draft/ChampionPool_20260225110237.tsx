...
const getDamageType = (profile: { ad: number; ap: number; true: number }) => {
  ...
  if (t >= 0.1) return { label: "TRUE", style: "bg-zinc-100 text-zinc-900 border-transparent", textStyle: "" };
  if (ad > 0.3 && ap > 0.3) return { 
    label: "HYBRID", 
    style: "bg-gradient-to-r from-orange-500 to-blue-500 border-transparent shadow-lg", 
    textStyle: "text-white font-black" 
  };
  if (ap > 0.6) return { label: "AP", style: "bg-blue-600 text-white border-transparent", textStyle: "" };
  return { label: "AD", style: "bg-orange-600 text-white border-transparent", textStyle: "" };
};
