...
import { TIER_COLORS } from "./ChampionPool";
  ...
  return (
    <Dialog>
      <DialogTrigger asChild>
        <button className="w-5 h-5 flex items-center justify-center rounded-full border border-white/10 bg-white/5 hover:bg-white/10 transition-colors shrink-0">
          <Info className="w-3 h-3 text-zinc-400" />
        </button>
      </DialogTrigger>
      <DialogContent className={cn(
        "max-w-3xl border-white/10 max-h-[90vh] overflow-y-auto",
        settings.disableTransparency ? "bg-zinc-900" : "bg-zinc-950"
      )}>
        <DialogHeader>
          <div className="flex items-center gap-3">
            ...
            <div>
              <DialogTitle className="text-2xl font-black tracking-tight text-white capitalize italic">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mt-6">
          ...
          <div className="space-y-6">
            <h3 className="text-xs font-black capitalize tracking-[0.2em] text-blue-400 flex items-center gap-2">
          ...
          <div className="space-y-8">
            ...
            <div className="space-y-4">
              ...
              <div className="h-6 w-full flex rounded-full overflow-hidden border border-white/5 shadow-inner">
                <div style={{ width: `${adPct}%` }} className="bg-orange-500 h-full transition-all duration-700 ease-out" title={settings.showTooltips ? `AD: ${Math.round(adPct)}%` : undefined} />
                <div style={{ width: `${apPct}%` }} className="bg-blue-500 h-full transition-all duration-700 ease-out" title={settings.showTooltips ? `AP: ${Math.round(apPct)}%` : undefined} />
                <div style={{ width: `${truePct}%` }} className="bg-white h-full transition-all duration-700 ease-out" title={settings.showTooltips ? `True: ${Math.round(truePct)}%` : undefined} />
              </div>
              <div className="flex justify-between text-[10px] font-black capitalize tracking-widest px-1">
            ...
            <div className="space-y-4">
              ...
              <div className={cn(
                "border border-white/5 rounded-xl p-4 space-y-3",
                settings.disableTransparency ? "bg-zinc-800" : "bg-black/15"
              )}>
              ...
              <div className="flex flex-wrap gap-2">
                {dominantTags.map(([tag, freq]) => (
                  <div key={tag} className="px-3 py-1.5 bg-blue-600/10 border border-blue-500/20 rounded-lg flex items-center gap-2">
                    <span className="text-[10px] font-black tracking-tighter text-blue-400">{formatTag(tag)}</span>
        ...
        <div className="mt-8 pt-8 border-t border-white/5">
           <div className="flex items-center justify-between gap-4">
              <div className="flex -space-x-3 overflow-hidden p-1">
                 {teamChampions.map((c) => (
                    <div key={c.id} className="relative w-10 h-10 rounded-full border-2 border-zinc-950 overflow-hidden bg-zinc-900 shadow-xl group">
                       <ChampionIcon name={c.name} url={c.iconUrl} className="w-full h-full" />
                       <div className={cn(
                         "absolute top-0 left-0 px-0.5 py-[1px] bg-black/90 rounded-br-sm border-r border-b border-white/10 shadow-sm z-10",
                         TIER_COLORS[c.tier]?.split(" ")[0] || "text-zinc-500"
                       )}>
                         <span className="text-[6px] font-black block leading-none">{c.tier}</span>
                       </div>
                    </div>
                 ))}
              ...
              <div className="text-right">
                 <div className="text-[10px] font-black text-zinc-500 capitalize tracking-widest mb-0.5">Total Composition Power</div>
    ...
    <div className="group cursor-default">
      <div className="flex items-center justify-between mb-1.5">
        <div className="flex items-center gap-2">
           {icon}
           <span className="text-[11px] font-bold text-zinc-100 capitalize tracking-tight">{label}</span>
        </div>
      ...
      <div className={cn(
        "max-h-0 overflow-hidden group-hover:max-h-10 transition-all",
...
function EngineRow({ label, value, description, isRisk }: { label: string, value: number, description: string, isRisk?: boolean }) {
  ...
  return (
    <div className="flex items-center justify-between group">
      <div>
        <div className="text-[10px] font-black capitalize text-zinc-200 tracking-tight">{label}</div>
