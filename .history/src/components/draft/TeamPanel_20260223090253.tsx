        ...
        {(Object.keys(ROLE_LABELS) as Role[]).map((role) => {
          ...
          return (
            <div key={role} className="flex flex-col gap-1">
                  ...
                  <div className={cn("flex flex-col flex-1 min-w-0", !isAlly && "items-end")}>
                    ...
                    {champion ? (
                      <div className={cn("flex flex-col", !isAlly && "items-end")}>
                        <span className="text-sm font-bold text-zinc-100 truncate">{champion.name}</span>
                        {bd && (
                          <div className={cn("flex gap-2 min-w-0 mt-0.5 text-[9px] font-bold", !isAlly && "flex-row-reverse")}>
                            <span className="text-emerald-400">{Math.round(champion.winrate)}% WR</span>
                ...
                {/* Synergies & Counters */}
                {champion && bd && (
                    <div className="mx-2 p-2 bg-black/20 border-x border-b border-white/5 rounded-b-lg flex flex-col gap-2">
                        <div className="flex gap-2 justify-between">
                            <div className={cn("flex flex-col gap-1 w-1/2", !isAlly && "items-end")}>
                                <span className={cn("text-[8px] font-bold uppercase", isAlly ? "text-emerald-500" : "text-orange-500")}>
                                     {isAlly ? "Top Synergies" : "Top Counters"}
                                </span>
                                <div className={cn("flex gap-1", !isAlly && "flex-row-reverse")}>
                                    {isAlly 
                                        ? getTopSynergies(champion).map(s => {
                                            const c = allChampions.find(champ => champ.id === s.id);
                                            return c ? <ChampionIcon key={c.id} name={c.name} url={c.iconUrl} className="w-5 h-5 rounded border border-white/10" /> : null;
                                        })
                                        : getTopCounters(champion.id).map(c => {
                                            const champ = allChampions.find(ch => ch.id === c.id);
                                            return champ ? <ChampionIcon key={champ.id} name={champ.name} url={champ.iconUrl} className="w-5 h-5 rounded border border-white/10" /> : null;
                                        })
                                    }
                                </div>
                            </div>
                            <div className={cn("flex flex-col gap-1 w-1/2", !isAlly && "items-end")}>
                                <span className={cn("text-[8px] font-bold uppercase", isAlly ? "text-orange-500" : "text-emerald-500")}>
                                    {isAlly ? "Weak Against" : "Synergizes With"}
                                </span>
                                <div className={cn("flex gap-1", !isAlly && "flex-row-reverse")}>
                                    {isAlly 
                                        ? getTopCounters(champion.id).map(c => {
      ...
      <Card className="mt-2 border-white/5 bg-zinc-900/80 backdrop-blur-xl overflow-hidden flex-1 shadow-2xl flex flex-col">
        <div className={cn("p-3 border-b border-white/10 flex justify-between items-center", bgColor, !isAlly && "flex-row-reverse")}>
            ...
            {teamChampions.length > 0 && (
               <div className="flex items-center gap-2 border border-white/10 bg-black/40 rounded px-2 py-0.5 shadow-inner">
                  <span className={cn("text-[10px] font-bold uppercase", labelColor)}>Grade</span>
                  <span className={cn(
                    "text-sm font-black text-transparent bg-clip-text bg-gradient-to-r",
                  ...
                  <span className="text-[10px] font-bold tracking-tight text-white">{Math.round(totalScore)}<span className="text-zinc-500">/500</span></span>
               </div>
            )}
        </div>
