...
export function ChampionPool({ champions }: ChampionPoolProps) {
  ...
  const bannedIds = useMemo(() => {
    return new Set<string>([...bans.ally, ...bans.enemy]);
  }, [bans]);
  ...
  const unavailableIds = useMemo(() => {
    return new Set<string>([...bannedIds, ...pickedIds]);
  }, [bannedIds, pickedIds]);
  ...
  const isTop3Visible = !activeSearch && !isBanMode;
    ...
    <div className="flex flex-col h-full gap-4 text-white font-sans selection:bg-blue-500/30">
      ...
      {top3.length > 0 && !activeSearch && !isBanMode && (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
          {top3.map((champion, idx) => {
            ...
            let champGrade = "D";
            if (finalScore >= 75) champGrade = "S+";
            else if (finalScore >= 68) champGrade = "S";
            else if (finalScore >= 60) champGrade = "A";
            else if (finalScore >= 52) champGrade = "B";
            else if (finalScore >= 44) champGrade = "C";
            ...
            return (
              <Card 
                ...
                className={cn(
                  ...
                  idx === 0 ? "border-cyan-500/30 scale-[1.02] z-10 shadow-cyan-500/5" : "border-white/5"
                )}
              >
                    ...
                    <div className="flex flex-col flex-1 min-w-0 pr-1">
                      <div className="flex justify-between items-center mb-0.5 border-b border-white/5 pb-0.5">
                        ...
                        <div className="flex items-center gap-1.5 overflow-hidden">
                           {idx === 0 && (
                             <span className={cn(
                               "text-[10px] font-black px-1 rounded-sm shadow-sm",
                               champGrade === "S+" || champGrade === "S" ? "bg-yellow-400 text-black" :
                               champGrade === "A" ? "bg-emerald-500 text-white" :
                               champGrade === "B" ? "bg-blue-500 text-white" :
                      ...
                      {score && (
                        <div className="flex flex-col gap-[3px] text-[9px] mt-0.5">
                          ...
                          <div className="flex justify-between items-center">
                            <span className="text-[7px] text-zinc-500 uppercase font-black tracking-widest leading-none">Counter</span>
                            <span className="font-bold text-emerald-500 leading-none">+{Math.round(score.breakdown.counter)}</span>
                          </div>
                          <div className="flex justify-between items-center">
                          ...
                          <div className="flex justify-between items-center">
                            <span className="text-[7px] text-zinc-500 uppercase font-black tracking-widest leading-none">Threat</span>
                            <span className="font-bold text-orange-400 leading-none">+{Math.round(score.breakdown.threat)}</span>
                          </div>
                          <div className="flex justify-between items-center">
                            <span className="text-[7px] text-zinc-500 uppercase font-black tracking-widest leading-none">Flex</span>
                            <span className="font-bold text-purple-400 leading-none">+{Math.round(score.breakdown.flexibility)}</span>
                          </div>
                          <div className="flex justify-between items-center">
                            <span className="text-[7px] text-zinc-500 uppercase font-black tracking-widest leading-none">Risk</span>
                            <span className="font-bold text-red-500 leading-none">-{Math.round(score.breakdown.risk)}</span>
                          </div>
                        </div>
                      )}
                    </div>
      ...
      <ScrollArea className="flex-1 bg-black/20 rounded-xl border border-white/5 p-5 shadow-inner">
        <div className="grid grid-cols-10 gap-2 justify-center items-start">
          {others.map((champion) => {
            const score = scoredMap.get(champion.id);
            const isBanned = bannedIds.has(champion.id);
            ...
            return (
              <button
                ...
                className={cn(
                  ...
                  isUnavailable ? (isPicked ? "opacity-20 grayscale cursor-not-allowed" : "cursor-not-allowed") : "hover:scale-105"
                )}
              >
                <div className="relative w-full aspect-square shadow-md group-hover:shadow-lg rounded-lg">
                  <div className={cn(
                    ...
                    <ChampionIcon 
                      ...
                      className={cn("w-full h-full", isUnavailable && "grayscale", isPicked && "opacity-30", isBanned && "opacity-50")} 
                    />
                    {isBanned && (
