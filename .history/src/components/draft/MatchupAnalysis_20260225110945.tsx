...
function VerdictBar({ analysis }: { analysis: any }) {
    ...
    return (
        <div className="border border-cyan-500/20 rounded-lg bg-black/40 p-4 relative overflow-hidden flex flex-col gap-4 shadow-xl">
            {/* Background Accent */}
            <div className="absolute top-0 right-0 w-32 h-32 bg-cyan-500/5 blur-[60px] rounded-full -mr-16 -mt-16 pointer-events-none" />
            
            <div className="flex items-center gap-2 text-[10px] uppercase font-black text-cyan-400 tracking-[0.2em] mb-1">
                <Brain className="w-3.5 h-3.5" /> Synthetic Tactical Analysis
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div className="flex flex-col gap-2">
                    ...
                    <ul className="flex flex-col gap-2">
                        {analysis.strengths.map((v: string, i: number) => (
                            <li key={i} className="text-[10px] text-zinc-300 leading-relaxed pl-3 border-l border-emerald-500/30 font-medium">
                                {v}
                            </li>
                        ))}
                    </ul>
                ...
                {/* Weaknesses */}
                <div className="flex flex-col gap-2">
                    <div className="flex items-center gap-1.5 text-[9px] font-black uppercase text-red-400 tracking-wider">
                        <AlertTriangle className="w-3 h-3 text-red-500" /> Critical Risks
                    </div>
                    <ul className="flex flex-col gap-2">
                        {analysis.weaknesses.map((v: string, i: number) => (
                            <li key={i} className="text-[10px] text-zinc-300 leading-relaxed pl-3 border-l border-red-500/30 font-medium">
                                {v}
                            </li>
                        ))}
                        {analysis.weaknesses.length === 0 && (
                            <li className="text-[10px] text-zinc-500 italic pl-3 border-l border-zinc-500/20">No major strategic weaknesses identified.</li>
                        )}
                    </ul>
                </div>
                ...
                {/* Win Conditions */}
                <div className="flex flex-col gap-2">
                    <div className="flex items-center gap-1.5 text-[9px] font-black uppercase text-cyan-400 tracking-wider">
                        <Target className="w-3 h-3" /> Execution Plan
                    </div>
                    <ul className="flex flex-col gap-2">
                        {analysis.winConditions.map((v: string, i: number) => (
                            <li key={i} className="text-[10px] text-cyan-50/90 leading-relaxed pl-3 border-l border-cyan-500/50 font-bold">
                                {v}
                            </li>
                        ))}
                    </ul>
