...
type SortOption = "score" | "winrate" | "name";
...
export const ChampionPool: React.FC<ChampionPoolProps> = ({ champions }) => {
  ...
  const filteredChampions = useMemo(() => {
    return champions
      .filter((c) => {
        ...
        const matchesRole = !focusedRole || c.roles.includes(focusedRole);
  ...
  }, [champions, search, focusedRole, scoredMap, sortBy]);
      ...
      <div className="flex flex-col gap-2 p-3 bg-zinc-900/20 border-b border-zinc-900 shrink-0">
        ...
        <div className="flex items-center gap-2 px-1">
          ...
          <span className="text-[9px] font-black text-zinc-500 uppercase tracking-widest">
            Filtering: {focusedRole || "All Roles"}
          </span>
          {focusedRole && (
        ...
        <div className="grid grid-cols-3 xs:grid-cols-4 sm:grid-cols-5 md:grid-cols-6 lg:grid-cols-7 xl:grid-cols-8 gap-2 py-3">
          {filteredChampions.map((champion) => {
            ...
            return (
              ...
              >
                <div className="relative aspect-square w-full rounded border border-zinc-800 overflow-hidden bg-zinc-900 group-hover:border-zinc-700 transition-colors">
                  <img
                ...
                <span className="text-[9px] font-bold text-zinc-500 truncate w-full text-center group-hover:text-zinc-300">
                  {champion.name}
                </span>
              </button>
            );
