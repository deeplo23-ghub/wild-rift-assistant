...
export function MatchupAnalysis() {
  ...
  if (allyChamps.length > 0) {
      console.log("DEBUG allyChamp[0]:", allyChamps[0].name, {
          burst: allyChamps[0].burstScore,
          keys: Object.keys(allyChamps[0])
      });
  } 
...
function VerdictBar({ generateVerdict }: { generateVerdict: () => string[] }) {
    ...
    return (
        <div className="border border-purple-500/10 rounded-lg bg-black/20 p-3 relative overflow-hidden">
             <ul className="flex flex-col gap-1 list-disc pl-4 text-[10px] text-zinc-400 font-medium italic marker:text-purple-500/30">
