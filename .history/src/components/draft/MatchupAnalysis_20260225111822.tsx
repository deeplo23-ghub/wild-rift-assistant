...
export function MatchupAnalysis() {
  ...
  const generateAnalysis = () => {
    ...
    const isComplete = allyChamps.length === 5 && enemyChamps.length === 5;
    ...
    } else if (earlyDiff < -0.8 && enemyChamps.length > 0) {
      analysis.weaknesses.push("Laning Vulnerability: Opponent has significant lane priority. Ally lanes are at risk of being shoved in.");
    }
    ...
    } else if (scalingDiff < -0.8 && enemyChamps.length > 0) {
      analysis.weaknesses.push("Outscaled: The enemy draft has a higher power ceiling. Allies must look for mid-game closures.");
    }

    // --- 2. Combat Radius & Mobility ---
    const rangeDiff = ally.range - enemy.range;
    if (rangeDiff > 1.0) {
        analysis.strengths.push("Combat Radius: Superior range allows for effective siege and safe poke-down before engagement.");
    } else if (rangeDiff < -1.0 && enemyChamps.length > 0) {
        analysis.weaknesses.push("Range Disadvantage: Enemy can chip you down from distance; requires hard engage to win fights.");
    }

    if (ally.mobility > 7) {
        analysis.strengths.push("High Map Agency: Superior mobility allows for faster rotations, better collapses, and safer splitting.");
    }

    // --- 3. Combat Tactics (Burst vs Sustain / Engage vs Peel) ---
    const burstVsDurability = ally.burst - enemy.durability;
    if (burstVsDurability > 1.2) {
      analysis.strengths.push("Pick Potential: High burst against low durability. Can delete priority targets before teamfights start.");
    }

    const engageVsPeel = ally.engage - enemy.peel;
    if (engageVsPeel > 1.2) {
      analysis.strengths.push("Initiation Priority: Excellent tools to bypass enemy defenses and force favorable fight conditions.");
    } else if (engageVsPeel < -1.2 && enemyChamps.length > 0) {
      analysis.weaknesses.push("Engage Contained: Enemy team has high peel/disengage. Blind dives will likely be punished.");
    }

    if (ally.sustain > 6.5) {
        analysis.strengths.push("Attrition Endurance: High healing/sustain allows the team to stay on the map longer and win long skirmishes.");
    }

    // --- 4. Damage Balance ---
    if (allyDamage.ad > 85) {
      analysis.weaknesses.push("Damage Skew (AD): Team is too physical-heavy. Enemy armor stacking will significantly reduce impact.");
    } else if (allyDamage.ap > 85) {
      analysis.weaknesses.push("Damage Skew (AP): Team is too magic-heavy. High enemy Magic Resist will negate most damage.");
    }

    // --- 5. Special Utility ---
    if (enemyChamps.length > 0 && ally.waveclear - enemy.waveclear < -1.5) {
      analysis.weaknesses.push("Siege Vulnerability: Poor waveclear makes defending towers against enemy pushes difficult.");
    }
    
    if (ally.objective > 7) {
      analysis.winConditions.push("Dragon/Baron Priority: High objective burn speed. Use Baron as a bait or for fast ninja takes.");
    }

    if (ally.cc > 7) {
      analysis.strengths.push("CC Lockdown: Exceptional crowd control allows for layered CC chains and pick opportunities.");
    }

    // Add in-progress note
    if (!isComplete) {
        analysis.winConditions.push("Draft in Progress: Complete both teams for full strategic breakdown.");
    }
