...
const ENGINE_METRICS = [
  ...
  { key: "risk", label: "Draft Risk", icon: AlertTriangle, desc: "Pick redundancy and execution difficulty", isRisk: true },
];
      ...
      settings.disableTransparency ? "bg-zinc-950" : "bg-black/40 backdrop-blur-3xl"
        ...
        <div className="h-full flex flex-col p-4 overflow-hidden">
            ...
            <div className="grid grid-cols-2 gap-4 mb-4">
                <TeamOverview grade={allyGrade} damage={allyDamage} color="blue" />
                <TeamOverview grade={enemyGrade} damage={enemyDamage} color="red" isEnemy />
            </div>
                ...
                <div className="h-full overflow-y-auto pr-1 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none] flex flex-col gap-2 pb-8">
                    ...
                    <div className="flex flex-col gap-1.5 bg-black/20 p-2.5 rounded-lg border border-white/5">
                        ...
                        {ENGINE_METRICS.map(m => {
                            ...
                            const aWin = m.isRisk ? aVal < eVal : aVal > eVal;
                            const eWin = m.isRisk ? eVal < aVal : eVal > aVal;
                            
                            const aNumColor = aWin ? "text-blue-400" : "text-white/60";
                            const eNumColor = eWin ? "text-red-400" : "text-white/60";
                            const aBarColor = m.isRisk ? (aVal > 30 ? "bg-red-500/60" : "bg-blue-500/20") : (aWin ? "bg-blue-500" : "bg-blue-500/20");
                            const eBarColor = m.isRisk ? (eVal > 30 ? "bg-red-500/60" : "bg-red-500/20") : (eWin ? "bg-red-500" : "bg-red-500/20");
                            const iconColor = aWin ? "text-blue-500" : eWin ? "text-red-500" : "text-zinc-500";
                            
                            return (
                                  <div key={m.key} className="flex items-center gap-3 group h-7 cursor-help">
                                     <span className={cn("w-6 text-right font-black tabular-nums text-[12px] transition-colors duration-300", aNumColor)}>{aVal.toFixed(0)}</span>
                                     <div className="flex-1 h-2 rounded-full bg-zinc-900/50 border border-white/5 overflow-hidden flex justify-end">
                                         <div className={cn("h-full transition-all duration-300", aBarColor)} style={{ width: `${aVal}%` }} />
                                     </div>
                                     <div className="w-24 flex flex-col items-center justify-center relative h-7">
                                         <div className={cn("flex items-center gap-1.5 text-[9.5px] font-bold transition-colors duration-300 justify-center", iconColor)}>
                                             <m.icon className="w-3 h-3" /> {m.label}
                                         </div>
                                     </div>
                                     <div className="flex-1 h-2 rounded-full bg-zinc-900/50 border border-white/5 overflow-hidden flex justify-start">
                                         <div className={cn("h-full transition-all duration-300", eBarColor)} style={{ width: `${eVal}%` }} />
                                     </div>
                                     <span className={cn("w-6 text-left font-black tabular-nums text-[12px] transition-colors duration-300", eNumColor)}>{eVal.toFixed(0)}</span>
                                 </div>
                             );
                        })}
                    </div>
                    ...
                    <div className="flex flex-col gap-1 bg-black/20 p-2.5 rounded-lg border border-white/5">
                        ...
                        {STAT_METRICS.filter(m => isStatsExpanded || IMPORTANT_METRICS.includes(m.key)).map(m => {
                            ...
                            const aNumColor = aWin ? "text-blue-400" : "text-white/60";
                            const eNumColor = eWin ? "text-red-400" : "text-white/60";
                            const aBarColor = aWin ? "bg-blue-500" : "bg-blue-500/20";
                            const eBarColor = eWin ? "bg-red-500" : "bg-red-500/20";
                            const iconColor = aWin ? "text-blue-500" : eWin ? "text-red-500" : "text-zinc-500";
                            
                            return (
                                  <div 
                                    key={m.key} 
                                    className="flex items-center gap-3 group h-7 cursor-help"
                                    onMouseEnter={() => setHoveredStatMetric(m.key)}
                                    onMouseLeave={() => setHoveredStatMetric(null)}
                                  >
                                     <span className={cn("w-6 text-right font-black tabular-nums text-[12px] transition-colors duration-300", aNumColor)}>{aVal.toFixed(1)}</span>
                                     <div className="flex-1 h-2 rounded-full bg-zinc-900/50 border border-white/5 overflow-hidden flex justify-end">
                                     ...
                                     <div className="w-24 flex flex-col items-center justify-center relative h-7">
                                         ...
                                         {/* Tooltip */}
                                         <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 px-2 py-1.5 bg-zinc-950/95 border border-white/10 rounded-lg text-[9px] text-zinc-300 whitespace-nowrap opacity-0 group-hover:opacity-100 transition-all duration-200 pointer-events-none z-50 shadow-2xl backdrop-blur-md translate-y-1 group-hover:translate-y-0">
                                             {m.desc}
                                     ...
                                     <span className={cn("w-6 text-left font-black tabular-nums text-[12px] transition-colors duration-300", eNumColor)}>{eVal.toFixed(1)}</span>
                                 </div>
                             );
                        })}
...
function TeamOverview({ grade, damage, color, isEnemy }: any) {
    const { settings } = useDraftStore();
    const themeColor = color === "blue" ? "text-blue-500" : "text-red-500";
    
    return (
        <div className={cn(
            "bg-black/20 p-4 rounded-xl border border-white/5 flex flex-col gap-5 shadow-xl relative overflow-hidden",
            ...
            <div className={cn(
                "flex items-center gap-3 border border-white/10 rounded-lg px-2.5 py-1.5 shadow-inner scale-95 origin-left", 
                isEnemy ? "flex-row-reverse origin-right" : "flex-row",
                settings.disableTransparency ? "bg-zinc-900" : "bg-black/15"
            )}>
                 <span className={cn(
                   "text-[22px] font-black text-transparent bg-clip-text bg-gradient-to-br",
                   grade.grade.startsWith("S") ? "from-yellow-400 to-orange-400" :
                   grade.grade === "A" ? "from-emerald-400 to-emerald-600" :
                   grade.grade === "B" ? "from-blue-400 to-blue-600" :
                   "from-zinc-400 to-zinc-600"
                 )}>{grade.grade}</span>
                 
                 <div className="w-px h-5 bg-white/10 opacity-50"></div>
                 
                 <div className="flex items-baseline gap-1">
                    <span className="text-sm font-black text-white tabular-nums tracking-tighter">{grade.total.toFixed(1)}</span>
                    <span className="text-[9px] font-bold text-zinc-600 opacity-60">/500</span>
                 </div>

                 <div className="w-px h-5 bg-white/10 opacity-50"></div>

                 <div className="flex items-baseline gap-1">
                    <span className={cn("text-xs font-black tabular-nums tracking-tighter", grade.avg >= 60 ? "text-emerald-400" : grade.avg >= 50 ? "text-blue-400" : "text-zinc-400")}>
                        {grade.avg.toFixed(1)}
                    </span>
                    <span className="text-[8px] font-black text-zinc-600 uppercase tracking-widest opacity-60">Avg</span>
                 </div>
            </div>
            ...
            <div className="w-full mt-auto pt-5 relative">
                ...
                <div className="flex w-full mb-1.5 items-end">
                    <div style={{ width: `${damage.ad}%` }} className="flex justify-center shrink-0 overflow-visible">
                        {damage.ad >= 10 && (
                    ...
                    <div style={{ width: `${damage.ap}%` }} className="flex justify-center shrink-0 overflow-visible">
                        {damage.ap >= 10 && (
                            <div className="flex items-center gap-1.5 leading-none">
                    ...
                    <div style={{ width: `${damage.truePct}%` }} className="flex justify-center shrink-0 overflow-visible">
                        {damage.truePct >= 10 && (
                            <div className="flex items-center gap-1.5 leading-none">
