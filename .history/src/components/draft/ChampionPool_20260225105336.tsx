...
const getDamageType = (profile: { ad: number; ap: number; true: number }) => {
  ...
  if (t >= 0.1) return { label: "TRUE", style: "text-zinc-100 border-zinc-100/30" };
  if (ad > 0.2 && ap > 0.2) return { label: "HYBRID", style: "bg-gradient-to-r from-orange-400 to-blue-400 bg-clip-text text-transparent border-white/20 font-black" };
  if (ap > 0.6) return { label: "AP", style: "text-blue-400 border-blue-400/50" };
  return { label: "AD", style: "text-orange-400 border-orange-400/50" };
};
      ...
      {top3.length > 0 && !activeSearch && !isBanMode && (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
          {top3.map((recChamp, idx) => {
            const score = scoredMap.get(recChamp.id);
            const isUnavailable = unavailableIds.has(recChamp.id);
            const topCounters = getTopCounters(recChamp.id);
            const topSynergies = getTopSynergies(recChamp);
            const topWeaknesses = getTopWeaknesses(recChamp.id);
            const finalScore = score ? score.finalScore : 0;
            const isPickedByAlly = pickedByAlly.has(recChamp.id);
            const isPickedByEnemy = pickedByEnemy.has(recChamp.id);
            const isPicked = isPickedByAlly || isPickedByEnemy;
            
            return (
              <Card 
                key={recChamp.id}
                onClick={(e) => { e.stopPropagation(); !isUnavailable && autoAction(recChamp.id); }}
                className={cn(
                    ...
                    <div className="flex flex-col items-center gap-1.5 shrink-0">
                      <div className="relative w-[76px] h-[76px] rounded-lg overflow-hidden border border-white/10 shadow-inner group-hover:border-blue-500/50 transition-colors">
                        ...
                        <div className={cn(
                          ...
                          <span className="text-[7px] font-black block leading-none">{getDamageType(recChamp.damageProfile).label}</span>
                        </div>
                        {isPicked && (
                          <div className={cn(
      ...
      <ScrollArea className={cn(
        ...
        <div className="grid grid-cols-10 gap-2 justify-center items-start">
          {others.map((champion) => {
            ...
            // A champion is unavailable if it is already picked or banned by either team
            const isUnavailable = isPicked || combinedBans.has(champion.id);
            ...
            return (
              <Tooltip key={champion.id} delayDuration={0}>
