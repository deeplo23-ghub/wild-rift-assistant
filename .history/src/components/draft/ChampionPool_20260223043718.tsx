...
export default function ChampionPool() {
  ...
  const clearActiveSlot = useDraftStore((state) => state.clearActiveSlot);

  const handlePick = (championId: string) => {
    if (activeSlot) {
      pickChampion(activeSlot.side, activeSlot.role, championId);
      clearActiveSlot();
    }
  };
      ...
      <div className="flex flex-col gap-2 shrink-0">
        ...
        <div className="flex items-center justify-between gap-4">
          <Input
            ...
            className="bg-zinc-950/50 border-zinc-800 flex-1"
          />
          {activeSlot && (
            <span className="text-sm text-blue-400 animate-pulse font-medium shrink-0">
              Select for {activeSlot.side} {activeSlot.role}
            </span>
          )}
        </div>
      </div>
        ...
        <div className="grid grid-cols-4 sm:grid-cols-5 md:grid-cols-6 lg:grid-cols-8 gap-3 content-start">
          {filtered.map((champ: Champion) => (
            <div
              ...
              className="flex flex-col items-center gap-2 p-2 rounded-md border border-zinc-800 bg-zinc-950/50 hover:bg-zinc-800 hover:border-zinc-600 transition-colors cursor-pointer group"
            >
