...
export const ScoreBreakdown: React.FC<ScoreBreakdownProps> = ({ scoredChampion, championName }) => {
  ...
  return (
    <div className="flex flex-col bg-gray-950 border border-gray-800 rounded-none shadow-none overflow-hidden font-sans">
      {/* Metrics Header */}
      <div className="p-3 border-b border-gray-800 bg-gray-900 flex items-center justify-between rounded-none">
        <h3 className="text-sm font-bold text-gray-200 uppercase tracking-normal truncate">{championName}</h3>
        <span className="text-sm font-bold text-gray-100">{Math.round(finalScore)}</span>
      </div>
        ...
        {Object.entries(breakdown).map(([key, value]) => {
          ...
          return (
            <div key={key} className="flex flex-col gap-1 rounded-none">
              ...
              <div className="h-1.5 w-full bg-gray-900 rounded-none overflow-hidden border border-gray-800/50">
                <div 
                  className={cn(
                    "h-full transition-none",
                    isRisk ? "bg-gray-700" : "bg-gray-300"
                  )}
      ...
      <div className="p-3 bg-gray-950 space-y-2">
        <div className="text-xs font-bold text-gray-600 uppercase tracking-tight">Context Logic</div>
        <div className="flex flex-col gap-1.5">
          {explanations.slice(0, 3).map((exp, i) => (
            <div key={i} className="flex gap-2 items-start group">
               <div className="w-1 h-3 bg-gray-800 mt-0.5 shrink-0" />
               <p className="text-xs leading-snug text-gray-400 font-normal">{exp}</p>
            </div>
          ))}
          {explanations.length === 0 && (
            <p className="text-xs text-gray-700 italic">No significant context detected.</p>
          )}
        </div>
      </div>
    </div>
