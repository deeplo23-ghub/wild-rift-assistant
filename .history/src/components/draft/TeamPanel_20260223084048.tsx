  ...
  return (
    <div className="flex flex-col gap-4 h-full">
      ...
      <div className="space-y-3">
        ...
        <div className={cn("flex gap-1.5 h-12", !isAlly && "flex-row-reverse")}>
            {[...Array(5)].map((_, i) => {
                ...
                return (
                    <div key={i} className="flex-1 rounded border border-white/5 bg-black/40 overflow-hidden relative group">
                        ...
                        {champ && (
                           <div className={cn(
                             "absolute inset-x-0 bottom-0 py-0.5 text-[6px] font-bold text-center text-white italic",
      ...
      <Card className="mt-2 border-white/5 bg-zinc-900/80 backdrop-blur-xl overflow-hidden flex-1 shadow-2xl">
        <div className={cn("p-3 border-b border-white/10", bgColor, !isAlly && "text-right")}>
            <h3 className={cn("flex items-center gap-2 text-[10px] font-bold uppercase tracking-widest", labelColor, !isAlly && "justify-end")}>
                {isAlly ? <Brain className="w-3 h-3" /> : null}
                {isAlly ? "Team Intelligence" : "Threat Analysis"}
                {!isAlly ? <ShieldAlert className="w-3 h-3" /> : null}
            </h3>
        </div>
