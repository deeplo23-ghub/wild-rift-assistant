...
import { cn, getRoleIcon, getWinrateColor, formatTag } from "@/lib/utils";
    ...
    <Tooltip delayDuration={0}>
      <TooltipTrigger asChild>
        <button
          ...
          {...(settings.confirmPicks
            ? { onDoubleClick: (e: React.MouseEvent) => { e.stopPropagation(); onClick(champion.id); } }
            : { onClick: (e: React.MouseEvent) => { e.stopPropagation(); onClick(champion.id); } }
          )}
          className={cn(
...
const RecommendationCard = memo(({ 
  ...
  return (
    <Card 
      {...(settings.confirmPicks
        ? { onDoubleClick: (e: React.MouseEvent) => { e.stopPropagation(); !isUnavailable && onClick(recChamp.id); } }
        : { onClick: (e: React.MouseEvent) => { e.stopPropagation(); !isUnavailable && onClick(recChamp.id); } }
      )}
            ...
            <div className="relative w-[76px] h-[76px] rounded-lg overflow-hidden border border-white/10 shadow-inner group-hover:border-blue-500/50 transition-colors">
              ...
              {settings.showTierBadges && (
                <div className={cn(
                  ...
                  TIER_COLORS[recChamp.tier]?.split(" ")[0] || "text-zinc-500"
                )}>
                  <span className="text-[10px] font-black block leading-none">{recChamp.tier}</span>
                </div>
              )}
              {settings.showDamageType && (
                <div className={cn(
                  ...
                  <span className={cn("text-[9px] font-black block leading-none", getDamageType(recChamp.damageProfile).textStyle)}>
              ...
              <div className="flex flex-col gap-1.5 mt-1">
                {settings.showWinRates && (
                  <div className="flex justify-between items-center">
                    <span className="text-[8px] text-zinc-500 font-black tracking-widest leading-none">WIN</span>
                    <span className={cn("text-[11px] font-bold leading-none", getWinrateColor(recChamp.winrate))}>{recChamp.winrate.toFixed(1)}%</span>
                  </div>
                )}
                {settings.showPickRates && (
                  <div className="flex justify-between items-center">
                    <span className="text-[8px] text-zinc-500 font-black tracking-widest leading-none">PICK</span>
                    <span className="text-[11px] font-bold text-zinc-300 leading-none">{recChamp.pickRate.toFixed(1)}%</span>
                  </div>
                )}
                {settings.showBanRates && (
                  <div className="flex justify-between items-center">
                    <span className="text-[8px] text-zinc-500 font-black tracking-widest leading-none">BAN</span>
                    <span className="text-[11px] font-bold text-red-400 leading-none">{recChamp.banRate.toFixed(1)}%</span>
                  </div>
                )}
              </div>
            )}
        ...
        <div className="flex flex-col gap-2 mt-[2px] px-1 pb-0.5">
          ...
          {settings.showTags && (
            <div className="relative">
              <div className="flex gap-1 overflow-hidden">
                {recChamp.tags.map((tag: string) => (
                  <span key={tag} className="text-[7px] font-black px-1 py-0.5 rounded bg-white/5 text-zinc-500 border border-white/5 shrink-0 whitespace-nowrap">
                    {formatTag(tag)}
                  </span>
                ))}
              </div>
          ...
          <div className="flex gap-1 items-center overflow-x-auto scrollbar-none">
            {settings.showSynergyIcons && topSynergies.map((s: any) => {
              ...
              return (
                <div key={s.id} title={`Synergy: ${target?.name}`} className="w-5 h-5 shrink-0 rounded border border-emerald-500/50 shadow-[0_0_5px_rgba(16,185,129,0.2)] overflow-hidden">
            ...
            {settings.showCounterIcons && topCounters.map((c: any) => {
              ...
              return (
                <div key={c.id} title={`Counter: ${target?.name}`} className="w-5 h-5 shrink-0 rounded border border-red-500/50 shadow-[0_0_5px_rgba(239,68,68,0.2)] overflow-hidden">
                  <ChampionIcon name={target?.name || ""} url={target?.iconUrl || ""} className="w-full h-full" />
                </div>
              );
            })}
            ...
            {settings.showWeaknessIcons && topWeaknesses.map((w: any) => {
              ...
              return (
                <div key={w.id} title={`Weakness: ${target?.name}`} className="w-5 h-5 shrink-0 rounded border border-yellow-500/50 shadow-[0_0_5px_rgba(234,179,8,0.2)] overflow-hidden">
                  <ChampionIcon name={target?.name || ""} url={target?.iconUrl || ""} className="w-full h-full" />
                </div>
              );
            })}
...
export function ChampionPool({ champions }: ChampionPoolProps) {
  ...
  return (
    <div className="flex flex-col h-full min-h-0 gap-4 text-white font-sans selection:bg-blue-500/30">
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="flex p-1 bg-black/20 rounded-lg border border-white/5 relative">
              {(["score", "winrate", "pickrate", "banrate", "tier"] as SortMetric[]).map((m) => {
                ...
                if (m === "pickrate") display = "Pick";
                if (m === "banrate") display = "Ban";
      ...
      {settings.showTopRecommendations && top3.length > 0 && (
