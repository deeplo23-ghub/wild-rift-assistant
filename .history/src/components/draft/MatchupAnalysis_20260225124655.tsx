...
export function MatchupAnalysis() {
  ...
  const getDetailedRatings = (champs: Champion[]) => {
      const getAvg = (k: string) => calculateAverage(champs, k);
      return [
          { label: "Toughness", icon: Shield, value: getAvg("durabilityScore"), color: "text-blue-400" },
          { label: "Engage", icon: Zap, value: getAvg("engageScore"), color: "text-orange-400" },
          { label: "Control", icon: Crosshair, value: getAvg("ccScore"), color: "text-purple-400" },
          { label: "Burst", icon: Skull, value: getAvg("burstScore"), color: "text-red-400" },
          { label: "Scaling", icon: TrendingUp, value: getAvg("scalingScore"), color: "text-emerald-400" },
          { label: "Teamfight", icon: Users, value: getAvg("teamfightScore"), color: "text-sky-400" },
      ];
  };

  const allyRatings = getDetailedRatings(allyChamps);
  const enemyRatings = getDetailedRatings(enemyChamps);
      ...
      settings.disableTransparency ? "bg-zinc-950" : "bg-black/40 backdrop-blur-3xl"
        ...
        <div className="h-full flex flex-col p-4 overflow-hidden">
            ...
            <div className="grid grid-cols-2 gap-4 mb-4">
                <TeamOverview stats={allyEngine} damage={allyDamage} ratings={allyRatings} color="blue" />
                <TeamOverview stats={enemyEngine} damage={enemyDamage} ratings={enemyRatings} color="red" isEnemy />
            </div>
            ...
            <div className="flex-1 min-h-0 relative group/scroll overflow-hidden">
                <div className="h-full overflow-y-auto pr-1 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none] flex flex-col gap-2 pb-8">
                    <div className="flex flex-col gap-1 bg-black/20 p-2.5 rounded-lg border border-white/5">
                        <div className="flex items-center justify-center text-[10px] font-black text-white tracking-widest border-b border-white/5 pb-1.5 mb-1.5">
                            Statistics
                        </div>
...
function TeamOverview({ stats, damage, ratings, color, isEnemy }: any) {
    ...
    return (
        <div className={cn(
            ...
            {/* Part 2: Deep Rating Analysis */}
            <div className={cn("w-full flex flex-col gap-2.5", isEnemy && "items-end")}>
                <div className="flex items-center gap-2 text-[8px] font-black text-white tracking-[0.2em] opacity-60">
                     <Brain className="w-3 h-3 text-cyan-400" /> DEEP RATING ANALYSIS
                </div>
                
                <div className="grid grid-cols-3 gap-x-4 gap-y-3 w-full">
                    {ratings.map((r: any) => (
                        <div key={r.label} className={cn("flex flex-col gap-0.5", isEnemy && "items-end")}>
                            <div className={cn("flex items-center gap-1 opacity-50", isEnemy && "flex-row-reverse")}>
                                <r.icon className="w-2.5 h-2.5" />
                                <span className="text-[7px] font-black tracking-tighter uppercase">{r.label}</span>
                            </div>
                            <span className={cn("text-sm font-black tabular-nums leading-none", r.color)}>
                                {r.value.toFixed(1)}
                            </span>
                        </div>
                    ))}
