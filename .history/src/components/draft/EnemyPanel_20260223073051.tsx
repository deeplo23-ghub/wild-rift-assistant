    ...
    <div className="flex flex-col gap-4 h-full">
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-end gap-2">
            <h2 className="text-xl font-black uppercase tracking-tighter text-white font-sans">Enemy Team</h2>
        ...
        <div className="flex flex-row-reverse gap-1.5 h-12">
            {[...Array(5)].map((_, i) => {
                ...
                return (
                    <div key={i} className="flex-1 rounded border border-white/5 bg-black/40 overflow-hidden relative group">
                        {champ ? (
                            <img src={champ.iconUrl} className="w-full h-full object-cover grayscale opacity-40" />
                        ) : (
                        ...
                        {champ && (
                           <div className="absolute inset-x-0 bottom-0 py-0.5 bg-zinc-700 text-[6px] font-black text-center text-white uppercase italic">Discarded</div>
                        )}
                    </div>
                );
            })}
            ...
            <div key={role} className="flex flex-col gap-1">
                ...
                >
                  ...
                  <div className="w-10 h-10 rounded-md bg-zinc-800 border border-white/5 flex items-center justify-center overflow-hidden shrink-0">
                    {champion ? (
                      <img src={champion.iconUrl} alt={champion.name} className="w-full h-full object-cover grayscale group-hover:grayscale-0 transition-all" />
                    ) : (
                      <div className={cn("w-3 h-3 rounded-full border", isFocused ? "bg-red-500 border-red-400 animate-pulse" : "bg-zinc-700 border-zinc-600")} />
                    )}
                  </div>
                  
                  <div className="flex flex-col flex-1 min-w-0 items-end">
                    <span className="text-[8px] font-black text-red-500 uppercase tracking-widest leading-none mb-0.5">
                      {ROLE_LABELS[role]}
                    </span>
                ...
                {champion && bd && (
                    <div className="mx-2 p-2 bg-black/20 border-x border-b border-white/5 rounded-b-lg grid grid-cols-4 gap-2">
                         <div className="flex flex-col items-center">
                            <span className="text-[7px] font-black text-zinc-600 uppercase">Thr</span>
                            <span className="text-[10px] font-bold text-red-400">{Math.round(bd.breakdown.threat)}</span>
                        </div>
                        <div className="flex flex-col items-center border-x border-white/5">
                            <span className="text-[7px] font-black text-zinc-600 uppercase">Ctr</span>
                            <span className="text-[10px] font-bold text-orange-400">{Math.round(bd.breakdown.counter)}</span>
                        </div>
                        <div className="flex flex-col items-center border-r border-white/5">
                            <span className="text-[7px] font-black text-zinc-600 uppercase">Base</span>
                            <span className="text-[10px] font-bold text-zinc-400">{Math.round(bd.breakdown.base)}</span>
                        </div>
                        <div className="flex flex-col items-center">
                            <span className="text-[7px] font-black text-zinc-600 uppercase">Flex</span>
      ...
      <Card className="mt-2 border-white/5 bg-zinc-900/80 backdrop-blur-xl overflow-hidden flex-1 shadow-2xl">
        <div className="bg-red-500/10 p-3 border-b border-red-500/10 text-right">
            <h3 className="flex items-center justify-end gap-2 text-[10px] font-black uppercase tracking-widest text-red-400">
            ...
            <div className="space-y-3">
               <div className="space-y-1.5">
                   <div className="flex justify-between text-[8px] font-black uppercase text-zinc-500">
                       <span>Offensive Pressure</span>
                       <span>{Math.round(avgThreat * 5)}%</span>
                   </div>
                   <div className="h-1 w-full bg-zinc-800 rounded-full overflow-hidden">
                       <div className="h-full bg-red-600 transition-all duration-1000" style={{ width: `${avgThreat * 5}%` }} />
                   </div>
               </div>
               <div className="space-y-1.5">
                   <div className="flex justify-between text-[8px] font-black uppercase text-zinc-500">
            ...
            <div className="grid grid-cols-2 gap-2">
                <div className="p-2 bg-black/20 rounded border border-white/5 flex flex-col gap-1 text-right">
                    <div className="flex items-center justify-end gap-1.5">
                        <span className="text-[8px] font-black text-zinc-500 uppercase">Volatility</span>
                ...
                <div className="p-2 bg-black/20 rounded border border-white/5 flex flex-col gap-1 text-right">
                    <div className="flex items-center justify-end gap-1.5">
                        <span className="text-[8px] font-black text-zinc-500 uppercase">Control</span>
                ...
                <div className="p-2 bg-black/20 rounded border border-white/5 flex flex-col gap-1 text-right">
                    <div className="flex items-center justify-end gap-1.5">
                        <span className="text-[8px] font-black text-zinc-500 uppercase">Win Condition</span>
                ...
                <div className="p-2 bg-black/20 rounded border border-white/5 flex flex-col gap-1 text-right">
                    <div className="flex items-center justify-end gap-1.5">
                        <span className="text-[8px] font-black text-zinc-500 uppercase">Threat Level</span>
