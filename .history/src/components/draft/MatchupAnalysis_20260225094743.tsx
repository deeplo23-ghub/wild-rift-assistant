    ...
    return (
        <div className="h-full flex flex-col p-4 overflow-y-auto">
            ...
            <div className="grid grid-cols-2 gap-4 mb-4">
                <div className="bg-black/20 p-3 rounded-lg border border-white/5"><TeamDamageCompact damage={allyDamage} /></div>
                <div className="bg-black/20 p-3 rounded-lg border border-white/5"><TeamDamageCompact damage={enemyDamage} isEnemy /></div>
            </div>
            ...
            <div className="flex flex-col gap-1 flex-1">
                {STAT_METRICS.map(m => {
                    ...
                    const aBarColor = aWin ? "bg-blue-500" : tie ? "bg-blue-500/50" : "bg-blue-500/20";
                    const eBarColor = eWin ? "bg-red-500" : tie ? "bg-red-500/50" : "bg-red-500/20";
                    ...
                    return (
                        <div key={m.key} className="flex items-center gap-4 bg-black/20 px-4 py-2 rounded border border-transparent hover:border-white/5 transition-all">
                            <span className={cn("w-8 text-right font-black tabular-nums text-sm", aNumColor)}>{aVal.toFixed(1)}</span>
                            <div className="flex-1 h-3 rounded bg-zinc-900 border border-white/5 overflow-hidden flex justify-end">
                                <div className={cn("h-full transition-all", aBarColor)} style={{ width: `${(aVal/10)*100}%` }} />
                            </div>
                            
                            <div className={cn("w-24 flex items-center justify-center gap-1 text-[10px] uppercase font-black transition-colors", iconColor)}>
                                <m.icon className="w-3 h-3" /> {m.label}
                            </div>
                            
                            <div className="flex-1 h-3 rounded bg-zinc-900 border border-white/5 overflow-hidden flex justify-start">
                                <div className={cn("h-full transition-all", eBarColor)} style={{ width: `${(eVal/10)*100}%` }} />
                            </div>
                            <span className={cn("w-8 text-left font-black tabular-nums text-sm", eNumColor)}>{eVal.toFixed(1)}</span>
                        </div>
                    );
                })}
            </div>
...
function TeamDamageCompact({ damage, isEnemy }: any) {
    return (
        <div className={`flex flex-col gap-2 w-full mt-1 ${isEnemy ? "items-end" : "items-start"}`}>
             <div className="flex gap-2 w-full max-w-full">
                 <div style={{ width: `${damage.ad}%` }} className="h-1.5 bg-orange-500 rounded-full" />
                 <div style={{ width: `${damage.ap}%` }} className="h-1.5 bg-sky-500 rounded-full" />
                 <div style={{ width: `${damage.truePct}%` }} className="h-1.5 bg-white rounded-full" />
