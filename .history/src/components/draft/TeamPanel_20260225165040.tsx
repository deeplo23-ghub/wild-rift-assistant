...
const BanSlot = memo(({ i, side, champ, isActiveBanSlot, settings, onBanClick, onRemoveBan }: any) => {
  return (
    <button 
      onClick={(e) => {
        e.stopPropagation();
              ...
              <>
                ...
                <div className={cn("flex items-center gap-1", !isAlly && "flex-row-reverse")}>
                   {syns.map((s: any) => {
                     ...
                     return c ? (
                       <div key={c.id} className="w-5 h-5 rounded border border-emerald-500/50 overflow-hidden cursor-help" onMouseEnter={() => onHoverChamp(c.id)} onMouseLeave={() => onHoverChamp(null)}>
                   ...
                   {ctrs.map((c: any) => {
                     ...
                     return champ ? (
                       <div key={champ.id} className="w-5 h-5 rounded border border-red-500/50 overflow-hidden cursor-help" onMouseEnter={() => onHoverChamp(champ.id)} onMouseLeave={() => onHoverChamp(null)}>
                   ...
                   {weaks.map((c: any) => {
                     const champ = allChampions.find((ch: any) => ch.id === c.id);
                     return champ ? (
                       <div key={champ.id} className="w-5 h-5 rounded border border-yellow-500/50 overflow-hidden cursor-help" onMouseEnter={() => onHoverChamp(champ.id)} onMouseLeave={() => onHoverChamp(null)}>
                         <ChampionIcon name={champ.name} url={champ.iconUrl} className="w-full h-full" />
                       </div>
                     ) : null;
                   })}
...
export const TeamPanel: React.FC<TeamPanelProps> = ({ side }) => {
  ...
  // Helpers
  const isSynergisticOrCounter = (champion: Champion) => {
    ...
    if (isAlly) {
      ...
      const matrix = (counterMatrix as any).get ? (counterMatrix as any).get(hoveredChampionId) : (counterMatrix as any)[hoveredChampionId as any];
      if (!matrix) return false;
      const val = matrix.get ? matrix.get(champion.id) : matrix[champion.id];
      return val > 2;
    }
    return false;
  };
    ...
    <div className="flex flex-col gap-4 h-full min-h-0">
      ...
      <div className="flex flex-col gap-1.5">
        {(Object.keys(ROLE_LABELS) as Role[]).map((role) => {
          ...
          const highlighted = champion ? isSynergisticOrCounter(champion) : false;
