...
import { Brain, ShieldAlert, Sparkles, Sword, Scale, AlertTriangle, Wind, Zap, Target, Skull, Waves, Activity } from "lucide-react";
    ...
    <div className={cn(
      ...
      <div className="grid grid-cols-2 gap-2">
        <DashMetric 
          ...
          icon={isAlly ? <Sparkles className="w-2.5 h-2.5 text-blue-400" /> : <Zap className="w-2.5 h-2.5 text-red-400" />} 
          active={teamChamps.length > 0}
        />
        <DashMetric 
          label={isAlly ? "Balance" : "Control"} 
          value={isAlly ? getBalanceLabel() : (stats.cc >= 6 ? "High" : stats.cc >= 3 ? "Mixed" : "Low")} 
          icon={isAlly ? <Scale className="w-2.5 h-2.5 text-zinc-400" /> : <Activity className="w-2.5 h-2.5 text-blue-400" />} 
          active={teamChamps.length > 0}
        />
        <DashMetric 
          label={isAlly ? "Risk Level" : "Win Condition"} 
          value={isAlly ? getRiskLabel(avgRisk) : getWinCondition()} 
          icon={isAlly ? <AlertTriangle className="w-2.5 h-2.5 text-orange-400" /> : <Target className="w-2.5 h-2.5 text-yellow-500" />} 
          active={teamChamps.length > 0}
        />
        <DashMetric 
          label={isAlly ? "Scaling" : "Threat Level"} 
          value={isAlly ? getScalingLabel(stats.scaling) : (avgComp >= 70 ? "Critical" : avgComp >= 55 ? "High" : "Standard")} 
          icon={isAlly ? <Wind className="w-2.5 h-2.5 text-blue-300" /> : <Skull className="w-2.5 h-2.5 text-zinc-500" />} 
          active={teamChamps.length > 0}
        />
      </div>
