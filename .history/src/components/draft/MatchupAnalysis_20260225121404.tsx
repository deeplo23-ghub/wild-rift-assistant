...
function TeamOverview({ stats, damage, tags, color, isEnemy }: any) {
    ...
    return (
        <div className={cn(
            ...
            <div className={cn(
                "absolute top-0 w-full h-0.5 opacity-50",
                color === "blue" ? "bg-blue-500" : "bg-red-500",
                isEnemy ? "right-0" : "left-0"
            )} />

            {/* Part 1: Core Metrics Grid */}
            <div className="w-full grid grid-cols-2 gap-4 border-b border-white/5 pb-4">
                 ...
                 <EngineRow label="Neutralization" value={stats.threat} color="text-zinc-200" isEnemy={isEnemy} />
                 <EngineRow label="Risk Factor" value={stats.risk} color="text-orange-400" isEnemy={isEnemy} />
            </div>

            {/* Part 2: Composition & Damage */}
            <div className={cn("w-full flex gap-6", isEnemy && "flex-row-reverse")}>
                {/* Traits focus */}
                <div className={cn("flex-1 flex flex-col gap-2", isEnemy && "items-end")}>
                    <div className="flex items-center gap-1.5 text-[8px] uppercase font-black text-zinc-500 tracking-[0.2em] opacity-50">
                         <Tags className="w-3 h-3" /> Strat Focus
                    </div>
                    <div className={cn("flex flex-wrap gap-1.5", isEnemy && "justify-end")}>
                        {tags.map(([t, count]: any) => (
                            <div key={t} className={cn(
                                "px-1.5 py-0.5 rounded border flex items-center gap-1 transition-colors duration-300 hover:bg-white/5",
                                bgTheme
                            )}>
                                <span className={cn("text-[9px] font-black tracking-tight", themeColor)}>{formatTag(t)}</span>
                                <span className="text-[8px] font-bold text-white/40">x{count}</span>
                            </div>
                        ))}
                    </div>
                </div>
                ...
                {/* Damage Mix */}
                <div className={cn("shrink-0 w-24 flex flex-col gap-2", isEnemy && "items-end text-right")}>
                    <div className="flex items-center gap-1.5 text-[8px] uppercase font-black text-zinc-500 tracking-[0.2em] opacity-50">
                         <Swords className="w-3 h-3" /> Mix
                    </div>
                    <div className="w-full h-1.5 rounded-full overflow-hidden bg-black/40 flex">
                        <div style={{ width: `${damage.ad}%` }} className="bg-orange-500 h-full shadow-[0_0_8px_rgba(249,115,22,0.3)]" />
                        <div style={{ width: `${damage.ap}%` }} className="bg-sky-500 h-full shadow-[0_0_8px_rgba(14,165,233,0.3)]" />
                        <div style={{ width: `${damage.truePct}%` }} className="bg-white h-full shadow-[0_0_8px_rgba(255,255,255,0.2)]" />
                    </div>
                    <div className={cn("flex gap-2 text-[7px] font-black tabular-nums", isEnemy && "flex-row-reverse")}>
                        <span className="text-orange-500">{damage.ad.toFixed(0)}%</span>
                        <span className="text-sky-500">{damage.ap.toFixed(0)}%</span>
                        <span className="text-zinc-300">{damage.truePct.toFixed(0)}%</span>
                    </div>
                </div>
            </div>
        </div>
