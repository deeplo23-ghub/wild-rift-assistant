...
function VerdictBar({ analysis }: { analysis: any }) {
    ...
    return (
        <div className="border border-cyan-500/20 rounded-lg bg-black/40 p-4 relative overflow-hidden flex flex-col gap-3 shadow-xl">
            ...
            <div className="flex items-center gap-2 text-[10px] uppercase font-black text-cyan-400 tracking-[0.2em] mb-1 pb-2 border-b border-white/5">
                <Brain className="w-3.5 h-3.5" /> Synthetic Tactical Report
            </div>

            <ul className="flex flex-col gap-2.5">
                {allInsights.map((insight, i) => {
                    const config = {
                        win: { color: "text-cyan-400", marker: "border-cyan-500/50", dot: "bg-cyan-500", label: "WIN COND" },
                        risk: { color: "text-red-400", marker: "border-red-500/40", dot: "bg-red-500", label: "RISK" },
                        strength: { color: "text-emerald-400", marker: "border-emerald-500/30", dot: "bg-emerald-500", label: "STRENGTH" },
                    }[insight.type as keyof typeof analysis];

                    return (
                        <li key={i} className="flex gap-3 group">
                            <div className={cn("mt-1.5 w-1.5 h-1.5 rounded-full shrink-0", config?.dot || "bg-zinc-500")} />
                            <div className={cn("flex flex-col gap-0.5 border-l pl-3", config?.marker || "border-zinc-500/20")}>
                                <span className={cn("text-[7px] uppercase font-black tracking-widest opacity-60", config?.color)}>
                                    {config?.label}
                                </span>
