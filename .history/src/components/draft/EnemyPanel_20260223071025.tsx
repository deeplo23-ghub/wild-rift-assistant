  ...
  return (
    <Card className="h-full border-zinc-800 bg-zinc-950/50 flex flex-col overflow-hidden">
      <CardHeader className="p-3 border-b border-zinc-800 bg-zinc-900/40 shrink-0">
        <CardTitle className="flex flex-row-reverse items-center justify-between text-xs font-black uppercase tracking-widest text-zinc-400">
      ...
      <div 
        ...
        className={cn(
          ...
          banModeActive && focusedSide === TeamSide.Enemy 
            ? "bg-red-500/5 border-red-500/20 border-b-red-500/50" 
            : "border-zinc-800 bg-zinc-900/10"
        )}
      >
        <div className="flex flex-row-reverse gap-1 h-8">
          {[...Array(5)].map((_, i) => {
            ...
            return (
              <div 
                key={i} 
                className={cn(
                  "flex-1 rounded-sm bg-zinc-950 border transition-all overflow-hidden relative group",
                  champ ? "border-zinc-700" : "border-zinc-800/50"
                )}
              >
                {champ ? (
                   <>
                     ...
                     <div className="absolute top-0.5 left-0.5">
                        <Lock className="w-2 h-2 text-red-500/40" />
                     </div>
                   </>
                ) : (
                  <div className="h-full w-full flex items-center justify-center">
                    <span className="text-[8px] font-black text-zinc-800/50">{i + 1}</span>
                  </div>
                )}
              </div>
            );
        ...
        {ALL_ROLES.map((role) => {
          ...
          return (
            <button 
              ...
              className={cn(
                "group flex flex-row-reverse items-center gap-2 p-1.5 rounded-md border transition-all text-right",
                isFocused 
            ...
            >
              ...
              <div className="flex flex-col min-w-0 flex-1 items-end">
                <div className="flex flex-row-reverse items-center justify-between w-full gap-1">
                  ...
                  {!champion && isFocused && (
                    <Badge variant="outline" className="h-3 px-1 text-[7px] font-black border-red-500/50 text-red-500 uppercase bg-red-500/5 animate-pulse">
                      Selecting
                    </Badge>
                  )}
                </div>
              ...
              {champion && (
                <div className="flex flex-row-reverse items-center gap-2 shrink-0 pl-1">
                  <div className="flex flex-col items-start text-left">
                    <span className="text-[10px] font-black text-red-400 leading-none">
                      {Math.round(champion.winrate)}%
                    </span>
                    <span className="text-[7px] text-zinc-600 font-bold uppercase mt-0.5 text-left">WR</span>
                  </div>
                </div>
              )}
            </button>
      ...
      <div className="mt-auto p-3 border-t border-zinc-800 bg-zinc-900/20 space-y-3 shrink-0">
        ...
        {missingRoles.length > 0 && (
          <div className="flex items-center flex-row-reverse gap-1.5 px-2 py-1 rounded bg-amber-500/5 border border-amber-500/10">
        ...
        <div className="space-y-1.5">
          <div className="flex flex-row-reverse items-center justify-between text-[8px] font-black uppercase tracking-widest text-zinc-500">
          ...
          <div className="h-1 w-full bg-zinc-900 rounded-full overflow-hidden flex flex-row-reverse">
