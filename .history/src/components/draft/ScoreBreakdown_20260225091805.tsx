...
export const ScoreBreakdown: React.FC<ScoreBreakdownProps> = ({ scoredChampion, championName }) => {
  ...
  return (
    <Card className={cn(
      "border-white/10 h-full flex flex-col",
      settings.disableTransparency ? "bg-zinc-900" : "bg-black/15 backdrop-blur-xl"
    )}>
      <CardHeader className="pb-4">
        <CardTitle className="flex items-center justify-between">
          <div className="flex flex-col">
            <span className="text-xs font-black text-blue-500 capitalize tracking-widest leading-none mb-1">
              Engine Breakdown
            </span>
          ...
          <div className="flex flex-col items-end">
            ...
            <span className="text-[10px] text-slate-500 font-bold capitalize tracking-tighter mt-1">Draft Power</span>
          </div>
        </CardTitle>
      </CardHeader>
      ...
      <CardContent className="flex-1 flex flex-col gap-6 overflow-hidden">
          ...
          {Object.entries(breakdown).map(([key, value]) => {
            ...
            return (
              <div key={key} className="space-y-1.5">
                <div className="flex justify-between items-center text-[10px] font-bold capitalize tracking-widest">
                ...
                <Progress 
                  value={value} 
                  className={`h-1 bg-white/10 ${isPenalty ? "text-red-500" : "text-blue-500"}`} 
                />
              </div>
            );
          })}
        ...
        <div className="flex-1 flex flex-col min-h-0">
          <div className="flex items-center gap-2 mb-3 text-xs font-bold text-slate-300 capitalize tracking-widest">
            ...
            <div className="flex flex-col gap-2">
              {explanations.map((exp, i) => (
                <div 
                  key={i}
                  className={cn(
                    "p-3 text-xs leading-relaxed text-slate-400 rounded-lg border border-white/5",
                    settings.disableTransparency ? "bg-zinc-800" : "bg-white/5"
                  )}
                >
                  {exp}
                </div>
