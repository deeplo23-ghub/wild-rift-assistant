        ...
        <div className="flex gap-2">
          {(["all", Role.Baron, Role.Jungle, Role.Mid, Role.Dragon, Role.Support] as const).map((r) => (
            <button
              ...
              className={cn(
                "px-4 py-2 rounded-lg text-xs font-black uppercase tracking-widest border transition-all flex items-center gap-2",
                activeRoleFilter === r 
                      ...
                      <div className="flex justify-between items-start">
                        <div className="flex items-center gap-2 min-w-0">
                           ...
                           {activeRoleFilter !== "all" && (
                             <img src={getRoleIcon(activeRoleFilter)} className="w-3.5 h-3.5 object-contain brightness-200 opacity-50 shrink-0" alt={activeRoleFilter} />
                           )}
                        </div>
                        ...
                        {counters.map(c => {
                          ...
                          return (
                            <div key={c.id} title={target?.name} className="w-6 h-6 rounded border border-white/5 overflow-hidden">
                               <ChampionIcon name={target?.name || ""} url={target?.iconUrl || ""} className="w-full h-full" grayscale />
                            </div>
                          );
                        })}
                      </div>
