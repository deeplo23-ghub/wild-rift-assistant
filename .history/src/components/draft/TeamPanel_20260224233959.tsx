                      ...
                      <>
                        ...
                        <div className={cn("flex items-center gap-1", !isAlly && "flex-row-reverse")}>
                           {getTopSynergies(champion).slice(0, 3).map(s => {
                             ...
                             return c ? (
                               <div key={c.id} title={`Synergy: ${c.name}`} className="w-4 h-4 rounded border border-emerald-500/50 shadow-[0_0_5px_rgba(16,185,129,0.2)] overflow-hidden">
                           ...
                           {getTopCounters(champion.id).slice(0, 3).map(c => {
                             ...
                             return champ ? (
                               <div key={champ.id} title={`Counter: ${champ.name}`} className="w-4 h-4 rounded border border-purple-500/50 shadow-[0_0_5px_rgba(168,85,247,0.2)] overflow-hidden">
                           ...
                           {getTopWeaknesses(champion.id).slice(0, 3).map(c => {
                             const champ = allChampions.find(ch => ch.id === c.id);
                             return champ ? (
                               <div key={champ.id} title={`Weakness: ${champ.name}`} className="w-4 h-4 rounded border border-red-500/50 shadow-[0_0_5px_rgba(239,68,68,0.2)] overflow-hidden">
                                 <ChampionIcon name={champ.name} url={champ.iconUrl} className="w-full h-full" />
                               </div>
                             ) : null;
                           })}
