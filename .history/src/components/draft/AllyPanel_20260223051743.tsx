    ...
    <div className="flex flex-col gap-4 p-6 bg-slate-900/50 backdrop-blur-md rounded-2xl border border-blue-500/20 shadow-xl shadow-blue-500/5">
      ...
      <div className="flex flex-col gap-3">
        {(Object.keys(ROLE_LABELS) as Role[]).map((role) => {
          ...
          const isActive = activeSide === TeamSide.Ally && activeRole === role && !isBan;
          const score = slot.championId ? scoredMap.get(slot.championId)?.finalScore : null;
          ...
          return (
            ...
            >
              ...
              <div className="flex-1 overflow-hidden">
                <span className={`text-[10px] font-bold uppercase tracking-widest leading-none block mb-1 truncate ${isActive ? "text-blue-300" : "text-blue-400"}`}>
                  {ROLE_LABELS[role]}
                </span>
                <span className={`text-sm font-black truncate block ${slot.championId ? "text-slate-100" : "text-slate-500 italic font-medium"}`}>
              ...
              {slot.championId && (
                <div className="shrink-0 text-right">
                  ...
                  <div className="text-[10px] text-slate-500 font-bold uppercase tracking-tighter">Power</div>
                </div>
              )}
