...
export const ChampionPool: React.FC<ChampionPoolProps> = ({ champions }) => {
  ...
  return (
    <div className="flex flex-col h-full bg-gray-950 border-x border-gray-800 overflow-hidden relative rounded-none">
      ...
      <div className="flex flex-col p-2 bg-gray-900 border-b border-gray-800 shrink-0 gap-2 rounded-none">
        <div className="flex gap-2">
          ...
          <Select value={sortBy} onValueChange={(v) => setSortBy(v as SortOption)}>
            ...
            <SelectContent className="bg-gray-900 border-gray-800 rounded-none">
              <SelectItem value="score" className="text-sm rounded-none">Engine Power</SelectItem>
              <SelectItem value="winrate" className="text-sm rounded-none">Meta Winrate</SelectItem>
              <SelectItem value="synergy" className="text-sm rounded-none">Strategic Link</SelectItem>
              <SelectItem value="counter" className="text-sm rounded-none">Opposition</SelectItem>
              <SelectItem value="name" className="text-sm rounded-none">Alpha Order</SelectItem>
            </SelectContent>
          </Select>
        </div>
        ...
        <div className="flex items-center gap-2 px-1">
          ...
          <span className="text-xs font-medium text-gray-600 uppercase">
            {banModeActive ? `Strategic Denial Search` : `Filter: ${focusedRole || "Full Deployment"}`}
          </span>
            ...
            return (
              ...
              >
                <div className={cn(
                  ...
                  isUnavailable ? "border-gray-900" : (banModeActive ? "border-gray-400" : "border-gray-800 focus:border-gray-100")
                )}>
                  <img
                  ...
                  <div className="absolute top-0 left-0 px-1 bg-gray-900 border-r border-b border-gray-800">
                    <span className="text-[10px] font-bold text-gray-200">{champion.tier}</span>
                  </div>
                </div>

                {/* Champion Info Labels */}
                <div className="w-full flex flex-col items-center gap-0">
                  <span className="text-xs font-medium text-gray-300 group-hover:text-gray-100 uppercase truncate w-full text-center">
                    {champion.name}
                  </span>
                  <div className="flex gap-0.5 opacity-40">
                    {champion.roles.map(r => (
                        <span key={r} className="text-[8px] font-bold text-gray-500 uppercase">{r[0]}</span>
                    ))}
                  </div>
                </div>
              </button>
      ...
      {hoveredChampionData && (
        <div className="absolute bottom-4 right-4 w-[280px] z-[100] pointer-events-none rounded-none border border-gray-800 bg-gray-950 backdrop-blur-md">
