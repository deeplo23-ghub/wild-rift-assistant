...
function TeamOverview({ grade, damage, color, isEnemy }: any) {
    ...
    return (
        <div className={cn(
            ...
            {/* Refined Power Scorecard */}
            <div className={cn(
                "flex items-center gap-4 rounded-xl px-4 py-2 border border-white/5 shadow-2xl relative group/card", 
                isEnemy ? "flex-row-reverse" : "flex-row",
                settings.disableTransparency ? "bg-zinc-900" : "bg-black/30 backdrop-blur-md"
            )}>
                 {/* Rating Badge */}
                 <span className={cn(
                   "text-3xl font-black text-transparent bg-clip-text bg-gradient-to-br",
                   grade.grade.startsWith("S") ? "from-yellow-400 via-orange-400 to-yellow-600" :
                 ...
                 {/* Avg Stack */}
                 <div className={cn("flex flex-col -gap-1", isEnemy ? "items-end" : "items-start")}>
                    <span className="text-[7px] font-black text-zinc-500 tracking-[0.2em] uppercase opacity-60">AVG</span>
                    <span className={cn(
                 ...
                 {/* Very Big Total Power */}
                 <div className={cn("flex items-baseline gap-1", isEnemy ? "flex-row-reverse" : "flex-row")}>
                    <span className="text-3xl font-black text-white tabular-nums tracking-tighter drop-shadow-sm">
                        {grade.total.toFixed(0)}
                    </span>
                    <span className="text-[10px] font-black text-zinc-600 mb-1">/500</span>
                 </div>
            </div>
