...
export function ChampionPool({ champions }: ChampionPoolProps) {
  ...
  return (
    <div className="flex flex-col h-full min-h-0 gap-4 text-white font-sans selection:bg-blue-500/30">
      ...
      {/* Main Grid */}
      <ScrollArea className="flex-1 min-h-0 bg-black/20 rounded-xl border border-white/5 p-5 shadow-inner">
        <div className="grid grid-cols-[repeat(auto-fill,84px)] gap-x-4 gap-y-6 justify-center items-start">
          {others.map((champion) => {
            const score = scoredMap.get(champion.id);
