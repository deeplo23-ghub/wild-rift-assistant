    ...
    <div className={cn(
      ...
      <div className={cn("flex flex-col gap-4", !isAlly && "items-end")}>
        <div className={cn("flex items-center gap-3", !isAlly && "flex-row-reverse")}>
            ...
            <h2 className="text-2xl font-bold tracking-tighter text-white font-sans">
                ...
                return (
                    <TooltipProvider key={i} delayDuration={0}>
                      <Tooltip>
                        ...
                        <TooltipContent side="bottom" className="bg-zinc-900 border-white/10 text-zinc-300 font-bold text-xs font-sans">
              ...
              <CardContent className={cn("p-2.5 flex items-center gap-4", !isAlly && "flex-row-reverse")}>
                ...
                <div className={cn("flex flex-col flex-1 min-w-0", !isAlly && "items-end")}>
                  <div className={cn("flex items-center gap-2 mb-0.5", !isAlly && "flex-row-reverse")}>
                    <span className={cn(
                      "text-[10px] font-bold tracking-tight font-sans",
                      isAlly ? "text-blue-500" : "text-red-500"
                    )}>
                      {ROLE_LABELS[role]}
                    </span>
                    {champion?.tier && (
                      <Badge variant="outline" className="text-[9px] h-3.5 px-1.5 border-white/10 text-zinc-500 bg-white/5 font-black font-sans uppercase">
                        {champion.tier}
                      </Badge>
                    )}
                  </div>
                  <h4 className={cn(
                    "text-sm font-bold truncate transition-colors capitalize font-sans",
      ...
      <div className="mt-auto pt-8 border-t border-white/5 flex flex-col gap-6">
        <div className={cn("flex justify-between items-end px-1", !isAlly && "flex-row-reverse")}>
          <div className={cn("flex flex-col", !isAlly && "items-end")}>
            ...
            <h3 className="text-2xl font-bold tracking-tighter text-white capitalize font-sans">
              {metrics?.composition || "Evaluation Pending"}
            </h3>
          </div>
          {metrics && (
            <div className={cn("text-right", isAlly ? "text-right" : "text-left")}>
              <span className="block text-[10px] font-bold text-zinc-600 uppercase mb-1 font-sans">Win Probability</span>
              <span className={cn("text-2xl font-black leading-none italic tracking-tighter font-sans", isAlly ? "text-blue-500" : "text-red-500")}>
          ...
          <div className="grid grid-cols-3 gap-3">
            {[
              ...
              <div key={i} className="bg-zinc-900/60 rounded-xl p-3 border border-white/5 flex flex-col items-center shadow-lg">
                ...
                <span className="text-[9px] font-bold text-zinc-500 uppercase tracking-widest font-sans">{m.label}</span>
              </div>
            ))}
          </div>
        )}
