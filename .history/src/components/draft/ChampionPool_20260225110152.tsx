...
const getDamageType = (profile: { ad: number; ap: number; true: number }) => {
  ...
  if (t >= 0.1) return { label: "TRUE", style: "text-zinc-100 border-zinc-100/30", textStyle: "" };
  if (ad > 0.2 && ap > 0.2) return { 
    label: "HYBRID", 
    style: "bg-gradient-to-r from-blue-900/80 to-orange-900/80 border-white/20", 
    textStyle: "bg-gradient-to-r from-orange-400 to-blue-400 bg-clip-text text-transparent font-black" 
  };
  if (ap > 0.6) return { label: "AP", style: "text-blue-400 border-blue-400/50", textStyle: "" };
  return { label: "AD", style: "text-orange-400 border-orange-400/50", textStyle: "" };
};
