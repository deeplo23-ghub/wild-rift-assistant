...
export function ChampionPool({ champions }: ChampionPoolProps) {
  ...
  /* 
     Reorder logic:
     Initial top3 is [rank1, rank2, rank3]
     We want to display [rank2, rank1, rank3] 
  */
  const rawTop3 = isTop3Visible ? availableSorted.slice(0, 3) : [];
  const top3 = useMemo(() => {
    if (rawTop3.length < 3) return rawTop3;
    const [r1, r2, r3] = rawTop3;
    return [r2, r1, r3];
  }, [rawTop3]);
  const others = sortedChampions.filter(c => !rawTop3.some(t => t.id === c.id));
      ...
      {top3.length > 0 && !activeSearch && !isBanMode && (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
          {top3.map((champion, idx) => {
            ...
            const isRank1 = champion.id === rawTop3[0]?.id;
            const isRank2 = champion.id === rawTop3[1]?.id;
            const isRank3 = champion.id === rawTop3[2]?.id;
            ...
            return (
              <Card 
                ...
                className={cn(
                  ...
                  isRank1 && "border-yellow-500 animate-glow-gold scale-[1.05] z-10",
                  isRank2 && "border-zinc-400 opacity-90 scale-[1.01] z-0",
                  isRank3 && "border-orange-700 opacity-90 scale-[0.98] z-0"
                )}
              >
                {isRank1 && <div className="absolute inset-0 animate-shimmer-gold pointer-events-none opacity-40" />}
