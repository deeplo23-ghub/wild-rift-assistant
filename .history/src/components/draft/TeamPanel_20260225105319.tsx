                    ...
                    {champion ? (
                      <>
                        ...
                        <div className={cn(
                          ...
                          <span className="text-[7px] font-black block leading-none">{getDamageType(champion.damageProfile).label}</span>
                        </div>
                      </>
                    ) : (
                      <img 
                      ...
                      <>
                        ...
                        <div className={cn("flex items-center gap-1", !isAlly && "flex-row-reverse")}>
                           ...
                           {getTopCounters(champion.id).map(c => {
                             ...
                             return champ ? (
                               <div key={champ.id} title={`Counter: ${champ.name}`} className="w-5 h-5 rounded border border-red-500/50 shadow-[0_0_5px_rgba(239,68,68,0.2)] overflow-hidden">
                                 <ChampionIcon name={champ.name} url={champ.iconUrl} className="w-full h-full" />
                               </div>
                             ) : null;
                           })}
                           {getTopWeaknesses(champion.id).map(c => {
                             const champ = allChampions.find(ch => ch.id === c.id);
                             return champ ? (
                               <div key={champ.id} title={`Weakness: ${champ.name}`} className="w-5 h-5 rounded border border-yellow-500/50 shadow-[0_0_5px_rgba(234,179,8,0.2)] overflow-hidden">
                                 <ChampionIcon name={champ.name} url={champ.iconUrl} className="w-full h-full" />
                               </div>
                             ) : null;
                           })}
