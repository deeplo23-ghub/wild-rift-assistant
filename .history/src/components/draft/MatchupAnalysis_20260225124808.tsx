...
export function MatchupAnalysis() {
  ...
  const getGradingAnalysis = (champs: Champion[]) => {
      const breakdowns = champs.map(c => 
          scoredChampions.find(s => s.championId === c.id)?.breakdown
      ).filter(Boolean);
      ...
      return [
          { label: "Composition", icon: Dna, value: getAvg("composition"), color: "text-blue-400" },
          { label: "Synergy", icon: Zap, value: getAvg("synergy"), color: "text-emerald-400" },
          { label: "Counter", icon: Crosshair, value: getAvg("counter"), color: "text-orange-400" },
          { label: "Threat", icon: ShieldAlert, value: getAvg("threat"), color: "text-purple-400" },
          { label: "Base Power", icon: TrendingUp, value: getAvg("base"), color: "text-zinc-300" },
          { label: "Risk Factor", icon: AlertTriangle, value: getAvg("risk"), color: "text-red-400" },
      ];
  };

  const allyGrading = getGradingAnalysis(allyChamps);
  const enemyGrading = getGradingAnalysis(enemyChamps);
      ...
      settings.disableTransparency ? "bg-zinc-950" : "bg-black/40 backdrop-blur-3xl"
        ...
        <div className="h-full flex flex-col p-4 overflow-hidden">
            ...
            <div className="grid grid-cols-2 gap-4 mb-4">
                <TeamOverview stats={allyEngine} damage={allyDamage} grading={allyGrading} color="blue" />
                <TeamOverview stats={enemyEngine} damage={enemyDamage} grading={enemyGrading} color="red" isEnemy />
            </div>
...
function TeamOverview({ stats, damage, grading, color, isEnemy }: any) {
    const themeColor = color === "blue" ? "text-blue-500" : "text-red-500";
    
    return (
        <div className={cn(
            "bg-black/20 p-4 rounded-xl border border-white/5 flex flex-col gap-6 shadow-xl relative overflow-hidden",
            ...
            {/* Deep Grading Analysis */}
            <div className={cn("w-full flex flex-col gap-3", isEnemy && "items-end")}>
                <div className="flex items-center gap-2 text-[8px] font-black text-white tracking-[0.2em] opacity-60">
                     <Brain className="w-3 h-3 text-cyan-400" /> DEEP GRADING ANALYSIS
                </div>
                ...
                <div className="grid grid-cols-3 gap-x-4 gap-y-4 w-full">
                    {grading.map((r: any) => (
                        <div key={r.label} className={cn("flex flex-col gap-0.5", isEnemy && "items-end")}>
                            ...
                            <span className={cn("text-xs font-black tabular-nums leading-none", r.color)}>
                                {r.value.toFixed(1)}
                            </span>
                        </div>
                    ))}
