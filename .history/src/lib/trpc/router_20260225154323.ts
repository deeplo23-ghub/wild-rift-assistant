...
export const appRouter = router({
  ...
  syncDatabase: publicProcedure.mutation(async ({ ctx }) => {
    ...
    // We pass the job ID so both scripts can update their own progress
    const command = `npx tsx scripts/scrape.ts --job-id ${job.id} && npx tsx scripts/seed.ts --job-id ${job.id}`;
