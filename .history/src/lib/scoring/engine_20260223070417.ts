...
export function scoreChampion(
  ...
  // Generate specific match-up data
  const matchUps = generateMatchUps(champion, allies, enemies, counterMatrix);
...
/**
 * Generate specific match-up lists for UI display.
 */
function generateMatchUps(
  ...
  counterMatrix: CounterMatrix
): { counters: string[]; synergizesWith: string[]; weakAgainst: string[] } {
  const result = {
    counters: [] as string[],
    synergizesWith: [] as string[],
    weakAgainst: [] as string[],
  };

  const counterMap = counterMatrix.get(champion.id);

  // Analyze Enemy Matchups
  enemies.forEach(enemy => {
    const val = counterMap?.get(enemy.id) ?? 0;
    if (val >= 2) result.counters.push(enemy.name);
    if (val <= -2) result.weakAgainst.push(enemy.name);
  });

  // Analyze Ally Synergies (Simplified logic similar to computeSynergyScore)
  allies.forEach(ally => {
     // Check for high synergy scores in computeSynergyScore's rules
     // For UI, we just identify who is on the team if they contribute significantly
     // In a real app we'd check tags specifically.
     // For now, if they contribute to synergy score, we list them.
     result.synergizesWith.push(ally.name); 
  });
