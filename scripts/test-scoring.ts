...
async function main() {
  ...
  for (const match of rawCounters) {
    ...
    counterMatrix.get(match.championId).set(match.opponentId, match.value);
  }

  // Setup mock draft
  const draftState = createEmptyDraftState();
  (draftState.ally[Role.Baron] as any).championId = "garen";
  (draftState.ally[Role.Mid] as any).championId = "ahri";
  (draftState.enemy[Role.Jungle] as any).championId = "lee-sin";
  (draftState.enemy[Role.Baron] as any).championId = "darius";
