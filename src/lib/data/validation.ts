...
export const NormalizedChampionSchema = z.object({
  ...
  burstScore: z.number().min(0).max(10),
  rangeScore: z.number().min(0).max(10),
  sustainScore: z.number().min(0).max(10),
  teamfightScore: z.number().min(0).max(10),
  objectiveScore: z.number().min(0).max(10),
