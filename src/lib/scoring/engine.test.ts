...
describe("Scoring Engine Core", () => {
  ...
  it("scoreChampion is deterministic", () => {
    ...
    const s1 = scoreChampion(dummyChamp, draft.enemy, [dummyChamp], [], 1, new Map(), baseCtx, weights);
    const s2 = scoreChampion(dummyChamp, draft.enemy, [dummyChamp], [], 1, new Map(), baseCtx, weights);
  ...
  it("handles empty draft (no allies, no enemies, zero counters)", () => {
    ...
    const result = scoreChampion(dummyChamp, emptyDraft.enemy, [dummyChamp], [], 1, new Map(), baseCtx, weights);
  ...
  it("handles a complex populated draft correctly (boosts component coverage)", () => {
    ...
    const result = scoreChampion(lucian, draft.enemy, [nami, lucian], [caitlyn, lux], 4, cMap, baseCtx, weights);
    expect(result.finalScore).toBeGreaterThan(0);
  ...
  it("tests RiskPenalty triggering at 2+ allies (all AD scenario)", () => {
    ...
    const result = scoreChampion(candidate, draft.enemy, [adChamp, ally2], [], 3, new Map(), baseCtx, weights);
